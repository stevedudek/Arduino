...
#define micSamples 20   // how many mic readings
#define averagedOver 8 // how quickly new values affect running average. Bigger numbers mean slower
  ...
  if (!finger[pinkie].isPushed()) switch (show) {
    ...
    case 3: {  // Sound Activated
      ...
      if (!finger[index].isOn() && !finger[middle].isPushed()) {  // Doing nothing else?
        Equalizer(Wheel(currColor)); // Light up using microphone
      }
      break;
    }
...
void Equalizer(uint16_t c) {
  int i;
  int lightOn;  // max light on
  int value;    // the value read from the microphone each time
  long signal;  // the reading after DC offset
  long avgRead; // average reading
  long runningAvg; // Running average
  long sumOfSquares = 0;
  
  for (i=0; i < micSamples; i++) { // Take many readings and average them
    value = analogRead(soundPin);  // Read mic
    signal = value - 512;          // Remove DC offset
    signal *= signal;              // Square to make all values positive
    sumOfSquares += signal;        // Add to total
  }
  
  avgRead = sumOfSquares/micSamples; // Take average
  runningAvg = (((averagedOver-1)*runningAvg)+avgRead)/averagedOver;
  lightOn = map(runningAvg,0,200,0,strip.numPixels()-1);
  
  for (i=0; i < strip.numPixels(); i++) {
    if (i<lightOn) { strip.setPixelColor(numLights-i-1, c); }
