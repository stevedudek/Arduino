...
unsigned int BoxWait = 0; // How long it has been since the box was last touched#define MaxWait 50      // How long box's setting stay untouched
...
void loop() {
  ...
  if (CheckBox()) {     // Has Box been altered?
    BoxWait = 150-(pace*2);  // Yes, set count down until random shows can begin again
  ...
  if (!BoxWait) {
    ...
    if(!random(3)) {  // Color2
      color2 = color2 - pace;
      if (color2 <= 0) color2 = color2 + MaxColor;
    }
    if(!random(20)) { // Pacing
      if (pace++ > MaxPace) pace = 1;
    }
    if(!random(100)) { // Show
      if(!random(2)) { show = 1; // Mostly, we get the rainbow show
      } else { if (show++ >= MaxShow) show = 0; }
    }
  }
