...
void dim_fall(uint8_t c, boolean reversed) {
  led->dimAllPixels(1);  // Try different amounts
...
void morph_channels(uint8_t fract) {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    CRGB color = led[CHANNEL_A].getInterpHSV(color_b, color_a, fract);  // interpolate a + b channels

    if (SMOOTHING > 0) {
      // Smooth the LED's new color against the LED's current color
      color = led[CHANNEL_A].smooth_rgb_color(leds[i], color, SMOOTHING);
    }
