...
uint8_t center_pixel[] = { random8(NUM_LEDS), random8(NUM_LEDS) };

// Clocks and time
#define SHOW_DURATION 30  // seconds
#define FADE_TIME 2   // seconds to fade in. If FADE_TIME = SHOW_DURATION, then Always Be Fading
...
// rainbow_point
// 
void rainbow_point(uint8_t channel) {
  uint8_t i, dist;
  uint8_t band = sin8_C(shows[channel].getCycle());
  
  for (i = 0; i < NUM_LEDS; i++) {
    dist = get_distance(i, center_pixel[channel]);
    shows[channel].setPixeltoHue(i, band - dist);
  }
}
...
// shadow_point
// 
void shadow_point(uint8_t channel) {
  uint8_t i, dist;
  uint8_t band = sin8_C(shows[channel].getCycle());
  uint8_t hue = shows[channel].getForeColor();
  
  for (i = 0; i < NUM_LEDS; i++) {
    dist = get_distance(i, center_pixel[channel]);
    led[channel].setPixelColor(i, led[channel].gradient_wheel(hue, band - dist));
  }
}
