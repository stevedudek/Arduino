...
byte backColor = 168;   // Starting background color
#define MaxColor 255    // Colors go from 0 to 255


// Shows

int show = 7;       // Starting show
...
void randomcolors() {
  for (int i=0; i < numLights; i++) {
    setPixelColor(i, Wheel(random(MaxColor)));
  }
}
...
void rainbowshow(int cycles) {
  ...
  for (int count = 0; count < cycles; count++) {
    ...
    // Possibly new delay
    if (!random(10)) { if (wait++ > MAX_WAIT) wait = 0; }
  }
  clearWithFade();
}
...
// lightwave
//
// Just one pixel traveling from x to 0
...
void lightrunup() {
  ...
  if (pos < numLights) {
    setPixelColor(pos, Wheel(foreColor));  // Turning on lights one at a time
  } else {
    setPixelColor(numLights-(pos % numLights)-1, Color(0,0,0));   // Turning off lights one at a time
  }
...
int GetDelayTime(int wait) {
  int DelayValues[MAX_WAIT] = { 1, 2, 4, 6, 8, 10, 15, 20, 30, 50, 75, 100 };
  if (wait < MAX_WAIT) { return (DelayValues[wait]); } else { return (DelayValues[MAX_WAIT]); }
}
