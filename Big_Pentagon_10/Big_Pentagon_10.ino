...
long last_time;
#define SMOOTHING 48   // 0 = no smooth, lower the number = more smoothing
...
// Shows
#define NUM_SHOWS 20
#define START_SHOW_CHANNEL_A  13  // Startings shows for Channels A + B
...
void morph_channels(uint8_t fract) {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    if (led_number != XX) {
      CHSV color = led[CHANNEL_A].getInterpHSV(mask(led[CHANNEL_B].getInterpFrameColor(rotate_pixel(i, CHANNEL_B)), i, CHANNEL_B), 
                                               mask(led[CHANNEL_A].getInterpFrameColor(rotate_pixel(i, CHANNEL_A)), i, CHANNEL_A), 
