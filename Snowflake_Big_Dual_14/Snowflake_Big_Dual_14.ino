...
//  Complex Switch Axis
//
//  Dual_14: Trying to get xBee to work
//
...
// ToDo: Missing first/last pixel on sym4. Check.
...
void setup() {
  ...
  Serial.begin(9600);  // Serial1: xBee port
...
void speak_command(char command, int value) {
  ...
  Serial1.print(value);
  Serial1.print(COMMAND_COMMA);
}

void speak_end_command() {
  Serial1.println(COMMAND_PERIOD);  // Speak terminal period
}

void speak_channel(int i) {
  char command_channel = (i == 0) ? COMMAND_CHANNEL_A : COMMAND_CHANNEL_B ;
  Serial1.print(command_channel);
}
