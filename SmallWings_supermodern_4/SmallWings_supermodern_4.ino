...
// Shows
#define START_SHOW_CHANNEL_A  5  // Channels A starting show
...
void setup() {
  ...
  FastLED.addLeds<DOTSTAR, DATA_PIN, CLOCK_PIN, BGR>(leds, ACTUAL_LEDS);
//  FastLED.addLeds<WS2801, DATA_PIN, CLOCK_PIN, RGB>(leds, ACTUAL_LEDS).setCorrection(TypicalLEDStrip);
  ...
  for (uint8_t i = 0; i < ACTUAL_LEDS; i++) {
    leds[i] = BLACK;
  }
...
void morph_channels() {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    if (led_buffer[i].h == color.h && led_buffer[i].s == color.s && led_buffer[i].v == color.v) {
      ...
      smoothed_color = led[CHANNEL_A].smooth_rgb_color(leds[i], rgb_color, 4);
      
      leds[i] = smoothed_color;
      leds[ACTUAL_LEDS-i-1] = smoothed_color;
