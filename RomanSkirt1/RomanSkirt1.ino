...
//
//  Acts a Master Controller: transmits its current state
//
...
//#define numLights 18     // Centurion helmet
#define numLights 15     // Centurion skirt
...
// Timing variables for xBee communication - Not needed for Master
...
void setup() {
  ...
  // xBee communication - Not needed for Master
...
void loop() { 
  ...
  talkdelay(20);  // Important delay, also involves speaking! 
...
// talk delay
//
// Speak once and then run a normal timed delay
//

void talkdelay(int delaytime) {
  SpeakBox();
  delay(delaytime); // The remainder
}

//
// Speak Box
//
// Check current state values against old state values
// If a value has changed, send out a message to update by Xbee all controllers
//

void SpeakBox() {
  
  // foreColor
  if (oldforeColor != foreColor) {
    oldforeColor = foreColor;
    SendMessage(COMMAND_COLOR, foreColor);
  }
  
  // backColor
  if (oldbackColor != backColor) {
    oldbackColor = backColor;
    SendMessage(COMMAND_BACKCOLOR, backColor);
  }
  ...
  if (oldwait != wait) {
    oldwait = wait;
    SendMessage(COMMAND_DELAY, GetDelayTime(wait));
  }
}
...
// Sends out the commander number: only 9=All costumes
...
void SendMessage(char command, int value) {
  ...
  Serial.print('9');         // 0=No control, 1-2-3-etc=specific costume, 9=All costumes
  Serial.print(command);
  Serial.print(value);
  Serial.println(COMMAND_END);
  delay(20);  // In case several messages are sent - may not be necessary
} 
