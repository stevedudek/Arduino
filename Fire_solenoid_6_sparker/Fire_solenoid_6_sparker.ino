...
#define SPARK_FREQ 30   // seconds
#define SPARK_DURATION 10  // milliseconds

#define FLAME_WAIT_MIN (10 * CLOCK_FREQ)   // seconds
#define FLAME_WAIT_MAX (30 * CLOCK_FREQ)  // seconds

#define GET_READY_TIME (60 * CLOCK_FREQ)  // seconds
...
uint16_t clock_timer = 0;  // in 0.25 seconds
boolean is_ready_to_flame = false;
...
Task taskSpark(TASK_SECOND * SPARK_FREQ, TASK_FOREVER, &Spark);
...
void setup() {
  ...
  digitalWrite(SPARK_PIN, LOW);  // Turn flames off
  ...
  userScheduler.addTask(taskSpark);
  taskSpark.enable();
}
...
void Spark() {
  digitalWrite(SPARK_PIN, HIGH);  // Turn spark on
  delay(SPARK_DURATION);
  digitalWrite(SPARK_PIN, LOW);  // Turn spark off
}
...
void checkFlame() {
  ...
  if (in_flame_show) {
    ...
    clock_timer++;
    if (!is_ready_to_flame && clock_timer > GET_READY_TIME) {
      is_ready_to_flame = true;
      clock_timer = 0;
    }
    if (is_ready_to_flame && clock_timer == flame_off_time) {
