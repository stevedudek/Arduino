...
#define DELAY_TIME 5  // in milliseconds (20ms=50fps, 25ms=40fps)
...
#define VARIABLE_SPOKE_BRIGHT false
...
#define SMOOTHING_SHOWS_HUE    40   // Fastest full rainbow = DELAY_TIME * (255 / this value)
...
void draw_center(uint8_t channel) {
  ...
  // leds_buffer[channel][NUM_LEDS-1] = led[channel].getNextFrameColor(0);
  led[channel].setPixelColor(NUM_LEDS-1, led[channel].getNextFrameColor(0));
}
...
void draw_hub(uint8_t channel) {
  ...
  for (uint8_t i = 0; i < pitch; i++) {
    for (uint8_t j = 0; j < symmetries[symmetry[channel]]; j++) {
      ...
      color = led[channel].getNextFrameColor(1 + i);

      //// An intermediate buffer might be necessary. Showing older code below.
//      leds_buffer[channel][pixel] = color;
      led[channel].setPixelColor(pixel, color);
    }
  }
}
...
void draw_spokes(uint8_t channel) {
  ...
  for (i = 0; i < arm_length; i++) {
    ...
    color = led[channel].getNextFrameColor(hub_pixels + i);
