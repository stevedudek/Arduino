...
ShowList Shows = { allOn, morphChain, randomfill, randomcolors, twocolor, lightwave, 
                   sawtooth, noisyshow, bands };
...
//
// check_brightness - check to see whether brightness has capped
//
void check_brightness() {
  int total_luma = 0;
  for (int i = 0; i < NUM_LEDS; i++) { 
    total_luma += leds[i].getLuma(); 
  }
  total_luma /= NUM_LEDS;  // get average brightness per LED

  if (total_luma > BRIGHTNESS) {
    FastLED.setBrightness( 256 * BRIGHTNESS / total_luma );
  }
}
