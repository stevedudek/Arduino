...
// Shows
#define START_SHOW_CHANNEL_A  24  // Channels A starting show
...
void draw_objects_pulsing(uint8_t i, uint8_t obj) {
  if (shows[i].isShowStart()) {
    shows[i].setWait(1);  // Turns off morphing
  ...
  CHSV color1 = led[i].gradient_wheel(c1, beatsin8(show_freq * 1.5));
  CHSV color2 = led[i].gradient_wheel(c2, beatsin8(show_freq));
  CHSV color3 = led[i].gradient_wheel(c3, beatsin8(show_freq * 2));
...
void draw_objects(CHSV color1, CHSV color2, CHSV color3, uint8_t channel, uint8_t obj) {
  ...
  for (uint8_t i = 0; i < NUM_FACES / 2; i++) {
    for (uint8_t j = 0; j < NUM_SPOKES; j++) {
      ...
      shows[channel].setPixeltoColor(((6 + i) * (NUM_SPOKES + 2)) + j, color);
    }
  }
}
...
void set_propellor_two_color(CHSV hub_color, CHSV color1, CHSV color2, uint8_t channel) {
  if (shows[channel].isShowStart()) { shows[channel].makeWaitFaster(3); }
...
void hub_pulse_two_color(uint8_t i) {
  ...
  set_hubs(led[i].gradient_wheel(shows[i].getForeColor(), beatsin8(show_freq * 1.5)), i);
  set_spokes(led[i].gradient_wheel(shows[i].getBackColor(), beatsin8(show_freq)), i);
}

void spinners(uint8_t channel) {
  if (shows[channel].isShowStart()) {
    shows[channel].setWait(1);  // Turns off morphing
...
void light_color_band(uint8_t i) {
  band_effect(i, led[i].wheel(shows[i].getForeColor()), WHITE );
}

void white_color_band(uint8_t i) {
  band_effect(i, WHITE, led[i].wheel(shows[i].getBackColor()) );
}
...
void band_effect(uint8_t channel, CHSV foreColor, CHSV backColor) {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    fract = beatsin8(show_freq, 0, 255, 0, get_distance(i, center_pixel) );
