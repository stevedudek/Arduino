...
//  Complex Switch Axis
//
//  White Lightning - Goes off randomly (not communicated)
//
...
//
#define SNOWFLAKE_NUMBER 1  // 0 = Small Simple, 1 = Medium Pointy, 
                            // 2 = Large Complex, 3 = Medium Inward

#define SPOKE_LENGTH 10  // [8, 10, 12, 10]
#define HUB_LENGTH 2     // [1,  2,  1,  1]
...
#define HAS_LIGHTNING true
uint8_t curr_lightning = 0;
uint8_t max_lightning = 1;
...
void draw_center(uint8_t channel) {
  leds_buffer[channel][NUM_LEDS-1] = lightning(redden(led[channel].getInterpFrameColor(0)));
}
...
void draw_hub(uint8_t channel) {
  ...
  for (uint8_t i = 0; i < pitch; i++) {
    for (uint8_t j = 0; j < symmetries[symmetry[channel]]; j++) {
      ...
      leds_buffer[channel][pixel] = lightning(redden(color));
    }
  }
}
...
void draw_spokes(uint8_t channel) {
  ...
  for (i = 0; i < arm_length; i++) {
    ...
    color = lightning(redden(dim_spoke(color)));
...
//
CHSV lightning(CHSV color) {
  if (HAS_LIGHTNING && (curr_lightning > 0) ) {
    uint8_t increase = 255 - cos8( map(curr_lightning, 0, max_lightning, 0, 255));
    color.s -= increase;
    color.v += increase;
  }
  return color;
...
void decrease_lightning() {
  if (HAS_LIGHTNING && (curr_lightning > 0) ) {
    curr_lightning--;  // Reduce the current bolt
  } else {
    new_lightning();  // Try to start a bolt
  }
}

//
// make_lightning - initiate white lightning
//
void make_lightning(uint8_t new_light) {
  max_lightning = new_light;
  curr_lightning = new_light;
}

//
// new_lightning - chance of a new lightning bolt
//
void new_lightning() {
  if (HAS_LIGHTNING && (curr_lightning == 0) ) {
    if (random16(2000) == 1) {  // For 1000, ~1 bolt every 20 seconds
      uint8_t bolt_time = random(10, 50);
      make_lightning(bolt_time);
    }
  }
}
