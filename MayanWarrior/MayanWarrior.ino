...
//  Leg Greaves (2 x 6)
//
//  Hook up in the order above - or disconnect greaves if you don't want them
//
//  Chestpiece has 4 unlit spacer LEDs: 12 (+4) + 9 + 12 = 37 LEDs (25 without greaves)
//
...
#define numLights 33
#define symLights 17
#define allLights 37  // 4 unlit space LEDs

boolean USE_GREAVES = true;  // false = greaves have been disconnected

// Dimmer
boolean USE_DIMMER = false;  // false = turn off dimmer capability
...
Adafruit_WS2801 strip = Adafruit_WS2801(allLights, dataPin, clockPin);

// left chestpiece | left greave | tail | right greave | right chestpiece
byte PetalLookUp[allLights] = { 0,2,3,5,6,7,
                                25,26,27,28,29,30,
...
void morph_frame() {
   ...
   for (int i = 0; i < numLights; i++) {
     strip.setPixelColor(PetalLookUp[i], HSVinter24(current_frame[i], next_frame[i], fract));
   }
