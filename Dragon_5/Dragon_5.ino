...
uint8_t section_list[] = { SECTION_TAIL };

// SCALES { tail: 46 }
// SPACERS { tail: 13 }

#define NUM_SCALES   (46)
#define NUM_SPACERS  (13)
...
// num_scales, num_spacers, num_leds, num_columns, y-offset
...
#define DELAY_TIME 120  // in milliseconds. A cycle takes 3-5 milliseconds. 15+ is safe.
...
uint8_t fade_amount = 15;  // 0 = no fading, to 255 = always be fading
...
// scales = 108, LEDs = 127
const uint8_t a_column_table[] PROGMEM = {
  1, 0, 6, 0,   //  0
  1, 0, 7, 6,   //  1
  1, 0, 7, 13,  //  2
  1, 1, 7, 20,  //  3
...
// scales = 108, LEDs = 127
const uint8_t b_column_table[] PROGMEM = {
  1, 0, 6, 0,   //  0
  1, 0, 7, 6,   //  1
  1, 0, 7, 13,  //  2
  1, 1, 7, 20,  //  3
...
// scales = 108, LEDs = 127
const uint8_t tail_column_table[] PROGMEM = {
  1, 0, 6, 0,   //  0
  1, 0, 7, 6,   //  1
  2, 0, 7, 13,  //  2
  1, 2, 6, 20,  //  3
...
void morph_channels(uint8_t fract) {
  ...
  for (uint8_t section = 0; section < NUM_SECTIONS; section++) {
    ...
    for (uint8_t column = 0; column < get_section_table_value(section, SECT_NUM_COLUMNS_FIELD); column++) {
      ...
      for (uint8_t scale = 0; scale < get_column_table_value(column, COLUMN_HEIGHT_FIELD, section); scale++) {
...
uint8_t get_column_table_value(uint8_t column, uint8_t field_index, uint8_t section) {
...
//
uint8_t get_section_table_value(uint8_t section, uint8_t field_index) {
