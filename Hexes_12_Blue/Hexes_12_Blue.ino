...
uint8_t DELAY_TIME = 10;  // in milliseconds. A cycle takes 3-5 milliseconds, so make this 6+.
long last_time;
...
#define START_SHOW_CHANNEL_B 23  // Starting show for Channels B. One of these needs to be 0.
...
uint8_t FADE_TIME = 1;  // seconds to fade in. If FADE_TIME = SHOW_DURATION, then Always Be Fading
...
const uint8_t angle_lookup[] PROGMEM = {
  32,  36,  42,  48,  55,  64,  72,  79,  85, 91, 95, 
  27,  32,  37,  45,  54,  64,  73,  82,  90, 95, 100, 
  22,  26,  32,  40,  50,  64,  77,  87,  95, 101, 105, 
  15,  19,  24,  32,  45,  64,  82,  95,  103, 108, 112, 
   8,  10,  13,  19,  32,  64,  95,  108, 114, 117, 119, 
   0,  0,   0,   0,   0,   0,  127, 127, 127, 127, 127, 
 248, 246, 243, 237, 224, 192, 161, 148, 142, 139, 137, 
 241, 237, 232, 224, 211, 192, 174, 161, 153, 148, 144, 
 234, 230, 224, 216, 206, 192, 179, 169, 161, 155, 151,
 229, 224, 219, 211, 202, 192, 183, 174, 166, 161, 156, 
 224, 220, 214, 208, 201, 192, 184, 177, 171, 165, 161
};
...
void setup() {
  ...
  for (uint8_t i = 0; i < DUAL; i++) {
    ...
    shows[i].setWaitRange(20, 200, 180);
...
void set_show_variables(uint8_t i) {
  ...
  shows[i].setWaitRange(20, 200, 180);  // default
...
void radar(uint8_t c) {
  uint8_t thickness = show_storage[c][0];
  
  shows[c].fillBlack();
  
  for (uint8_t y = 0; y < MAX_COORD; y++) {
    for (uint8_t x = 0; x < MAX_COORD; x++) {
      uint8_t angle_delta = abs((shows[c].getCycle() % 256) - get_angle(x, y));
...
//
uint8_t get_angle(uint8_t x, uint8_t y) {
  return pgm_read_byte_near(angle_lookup + (y * MAX_COORD) + x);
}
