...
int GetDelayTime(int wait) {
  int DelayValues[maxWait] = { 40, 60, 80, 100, 150, 200, 300, 500, 750, 1000, 2000, 3000 };
...
uint8_t KeyPad() {
  ...
  // No Key pad for this program!
  //
  // Instead randomly hit the keys
  
  int rand = random(100000);
  
  if (rand == 0) return DOWN; // Change the show
  if (rand < 10) return UP;
  if (rand < 50) return LEFT;
  if (rand < 100) return RIGHT;
  return NONE;
  /*
