...
CHSV getInterpHSV(int i, CHSV c1, CHSV c2, uint8_t fract) {
  ...
  if (c1 == c2) {
    ...
    color = constrain_palette(i, c2);
    color.v = dim8_lin(scale8(color.v, 255 - fract));  // try dim8_raw, dim8_lin
  } else if (is_black(c2)) {
    color = constrain_palette(i, c1);
    color.v = dim8_lin(scale8(color.v, fract));  // try dim8_raw, dim8_lin
  } else {
