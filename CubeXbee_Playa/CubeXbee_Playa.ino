...
#define NumLEDs     6        // Sides
#define NumNeigh    4       // Neighbors
#define Solid_ID    2        // 1 = tet, 2 = cube, 3 = oct, 4 = dodec, 5 = icosa
#define MAX_SOLID   5
...
# define I_AM_MASTER false; // true if master; false if slave
boolean TMP_MASTER = I_AM_MASTER;         // temporary buffer to hold master state
...
void loop() {
  ...
  if (HearSignal()) {  // Slave or hearing from the box
    TMP_MASTER = false;  // turn off master signalling
    update = true;
  }
...
//
void SpeakSignal() {
  byte chapter = cycle / (MAX_CLOCK / 3); // Hopefully 0 or 1
  ...
  switch(chapter) {
    ...
    default:    // 2nd chapter - All solids together
...
boolean HearSignal() {
  ...
  if (!Serial.available()) {
    ...
    if((NewTime - OldTime) / 1000 > MAX_AWAY) {
      ...
      TMP_MASTER = I_AM_MASTER;  // Restore solid mastery
...
boolean ProcessMessage(char* message) {
  ...
  if (solid == 0 || solid == Solid_ID) {  // Command for me
