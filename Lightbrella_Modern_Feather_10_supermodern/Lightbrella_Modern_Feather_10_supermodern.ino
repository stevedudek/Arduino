...
uint8_t fade_amount = 96;  // 0 = no fading, to 255 = always be fading
...
void dim_fall(uint8_t c, boolean reversed) {
  if (shows[c].isShowStart()) {
    ...
    param[c] = random(1, 10);
    shows[c].pickRandomCycleDuration(20, 300);
  }
...
void juggle_fastled(uint8_t c) {
  ...
  shows[c].dimAllPixels(20); // replaces: fadeToBlackBy( leds, NUM_LEDS, 20);
...
void morphChain(uint8_t c, boolean reversed) {
  ...
  for (uint8_t i = 0; i < SPOKE_LEDS; i++) {
    uint8_t color = map8(sin8_C((i + shows[c].getCycle()) * shows[c].getForeColorSpeed() * 0.5), 0, sin8_C(shows[c].getBackColor()));
