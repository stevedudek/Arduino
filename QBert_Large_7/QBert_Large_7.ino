...
boolean LIFE_BOARD[NUM_LEDS];  // Game of Life
...
void advance_clocks() {
  for (uint8_t i = 0; i < DUAL; i++) {
    ...
    if (shows[i].getSmallCycle() >= uint32_t(MAX_SMALL_CYCLE)) { 
      next_show(i);
      pick_next_show(i);
    }
  }
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    if (value) {
      ...
      if (pattern_type < 2) {
        ...
        uint8_t intense = get_wipe_intensity(XCOORDS[i], YCOORDS[i], show_variables[c + 10], (pattern_type < 4));
...
uint8_t get_pixel_distance_to_point(uint8_t i, uint8_t point_x, uint8_t point_y) {
  return get_distance_to_coord(XCOORDS[i], YCOORDS[i], point_x, point_y);
}
...
uint8_t lookup_complex_pattern_table(uint8_t n, uint8_t complex_number) {
  ...
  uint8_t ComplexMatrix[] = {
    13,13,13,12,12,12,11,11,11,10,10,10,14,14,14,5,4,4,3,3,2,2,1,1,9,9,9,15,15,15,6,5,6,6,7,8,9,10,11,18,18,17,8,8,8,16,
    16,16,7,7,8,3,5,4,1,12,6,14,12,13,16,17,16,7,7,7,17,17,17,9,8,9,1,2,18,17,15,16,15,15,14,6,6,6,18,18,18,10,10,11,11,
    12,12,13,14,13,5,5,5,1,1,1,2,2,2,3,3,3,4,4,4,1,1,1,5,5,5,1,1,1,7,7,7,2,2,2,4,4,4,2,2,2,6,6,6,1,1,1,3,3,3,3,3,3,3,3,
    3,5,5,5,2,2,2,5,5,5,4,4,4,2,2,2,3,3,3,4,4,4,3,3,3,4,4,4,1,1,1,1,1,1,4,4,4,3,3,3,4,4,4,3,3,3,2,2,2,5,5,5,2,2,2,5,5,5,
    2,2,2,3,3,3,1,1,1,6,6,6,1,1,1,4,4,4,0,0,0,3,0,4,0,0,0,9,0,10,10,0,9,0,2,1,6,5,11,0,8,7,6,11,5,5,11,6,7,8,0,0,7,8,9,
    10,0,0,1,2,3,4,0,0,4,3,2,1,0,10,0,11,5,0,4,0,0,0,10,0,9,0,0,0,0,0,0,0,9,8,7,6,1,2,3,0,5,11,6,7,8,0,0,0,0,4,0,5,11,0,
    10,0,4,3,2,1,0,0,3,2,1,6,7,8,9,0,0,0,0,0,0,6,7,0,8,9,0,10,11,0,0,8,7,6,0,5,4,0,0,0,13,12,0,3,4,5,1,9,2,3,5,4,0,3,2,
    15,14,0,1,2,9,8,6,7,0,0,11,12,0,13,14,0,15,1,1,1,2,0,3,4,0,5,6,0,0,0,10,9,0,0,0,3,2,0,0,15,14,0,0,0,8,7,0,0,8,7,5,4,
    0,9,1,2,0,13,12,10,9,0,0,6,0,7,8,6,5,3,4,0,11,0
};
  return ComplexMatrix[((complex_number % NUM_COMPLEX_PATTERNS) * NUM_LEDS) + n];
}

uint8_t get_complex_max(uint8_t complex_number) {
  uint8_t complex_max[] = { 18, 7, 11, 15 };
