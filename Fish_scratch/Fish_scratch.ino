...
CHSV getInterpHSV(int i, CHSV c1, CHSV c2, uint8_t fract) {
  ...
  if (c1 == c2) {
    ...
    color = constrain_palette(i, CHSV(c2.h, c2.s, interpolate(c1.v, c2.v, fract)));  // safe approach
//    color = constrain_palette(i, CHSV(c2.h, c2.s, dim8_raw(scale8(c2.v, fract))); // try this too
  } else if (is_black(c2)) {
    color = constrain_palette(i, CHSV(c1.h, c1.s, interpolate(c1.v, c2.v, fract)));  // safe approach
//    color = constrain_palette(i, CHSV(c1.h, c1.s, dim8_raw(scale8(c1.v, 255 - fract))); // try this too
  } else {
