...
#define NumLEDs    12        // Sides
#define NumNeigh    5        // Neighbors
#define Solid_ID    4        // 1 = tet, 2 = cube, 3 = oct, 4 = dodec, 5 = icosa
#define MAX_SOLID   5
...
#define MaxColor 3071   // Number of colors
...
void loop() {
  ...
  if (HearSignal()) update = true;  // Even Masters need to listen for the control box
  
  if (I_AM_MASTER && Alone) SpeakSignal();
...
//
void SpeakSignal() {
  byte chapter = cycle / (MAX_CLOCK / 3); // Hopefully 0, 1, 2
  byte currSolid;
  ...
  switch(chapter) {
    ...
    case 1:    // 2nd chapter - Lights flash in sequence
      if (morph == 0) {
        currSolid = 1 + (cycle % MAX_SOLID);  // 1 thru 5
        SendAllData(currSolid);
        
        if (currSolid == Solid_ID) {
          active = true;  // Bring back to life
        } else {
          active = false;
          BlackCones();  // Kill lights
        }
      }
      break;
      
    default:    // 3rd chapter - All solids together
...
boolean ProcessMessage(char* message) {
  ...
  if (solid == 0 || solid == Solid_ID) {  // We should be active!
    ...
    BlackCones();  // Turn off all lights
...
void morph_frame() {
  if (active) {  // Only works if solid is active!
     for (int i = 0; i < NumLEDs; i++) {
