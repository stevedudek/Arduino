...
uint8_t botColor = 20;     // Starting bottom-teeth color in 'int' form
...
void loop() {
  ...
  strip.show();                     // 1) Turn on lights
  delay(random(20,50)*100);         // 2) Keep lights on for a while
  //clearWithFade(random(5,20)*100);  // 3) Turn lights off slowly
...
void SetTopTeeth (uint8_t color) {
  if (color) {
    ...
    strip.setPixelColor(2, Wheel(color));
  } else {
...
void SetBottomTeeth (uint8_t color) {
  if (color) {
    ...
    strip.setPixelColor(3, Wheel(color));
  } else {
...
uint16_t Color(byte r, byte g, byte b)
{
  ...
  return( ((unsigned byte)g & 0x1F )<<10 | ((unsigned byte)b & 0x1F)<<5 | (unsigned byte)r & 0x1F);
}
...
void morph_frame(int msec) {
  ...
  byte deltas[numLights][3];
  for (int i = 0; i < numLights; i++) {
    for (int j = 0; j < 3; j++) {
      uint8_t old_color = (current_frame[i] >> (j * 5)) & 0x1F; 
      uint8_t new_color = (next_frame[i] >> (j * 5)) & 0x1F;
      deltas[i][j] = (unsigned byte)new_color - (unsigned byte)old_color;
    }
  }
...
void setPixelColor(byte pos, uint16_t color) {
