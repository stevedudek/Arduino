...
uint8_t DELAY_TIME = 15;  // in milliseconds. A cycle takes 3-5 milliseconds. 15+ is safe.
long last_time;
...
uint8_t FADE_TIME = 60;  // seconds to fade in. If FADE_TIME = SHOW_DURATION, then Always Be Fading
...
boolean is_speak_time(uint8_t i) {
  if (shows[i].isMorphing()) {
    return shows[i].getMorph() == 0;
  } else {
    return shows[i].getSmallCycle() % 25 == 0;
  }
}
...
void speak_channel_commands(uint8_t i) {
  ...
  m = speak_command(COMMAND_SYM, symmetry[i], m);
...
boolean GetMessage(char* message) {
  ...
  uint8_t tries = 0;
  uint16_t MAX_TRIES = 2000;  // 200 is too low
  uint8_t i = 0;
...
void ExecuteOrder(char command, uint16_t value, uint8_t i) {
  ...
  switch (command) {
    ...
    case COMMAND_FORE:
      shows[i].setForeColor(value % MAX_COLOR);
      break;
    
    case COMMAND_BACK:
      shows[i].setBackColor(value % MAX_COLOR);
      break;

    case COMMAND_BRIGHTNESS:
      if (BRIGHTNESS != value) {
        BRIGHTNESS = value;
        FastLED.setBrightness(BRIGHTNESS  % MAX_COLOR);
      }
      break;
...
void center_ring(uint8_t c) {
  ring(c, shows[c].getForeColor(), shows[c].getForeBlack(), 5, 5, 8, 64, 128);
}

void well(uint8_t c) {
  ring(c, shows[c].getForeColor(), led[c].wheel(shows[c].getBackColor()), 5, 5, 4, 192, 192);
}
...
void morph_channels(uint8_t fract) {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    if (led_number != XX) {
      CHSV color_b = led[CHANNEL_B].getInterpFrameColor(rotate_pixel(i, CHANNEL_B));
      CHSV color_a = led[CHANNEL_A].getInterpFrameColor(rotate_pixel(i, CHANNEL_A));
...
void mirror_pixel(uint8_t channel, uint8_t i, uint8_t rotations) {
  ...
  led[channel].setInterpFrame(new_pixel, led[channel].getInterpFrameColor(i));
}
