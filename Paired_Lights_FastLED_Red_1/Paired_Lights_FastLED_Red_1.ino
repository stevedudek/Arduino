...
// Palettes - RED part is here
#define CAN_CHANGE_PALETTES false
uint8_t palette_start = 195; // 0-255 (hue)
uint8_t palette_width = 50;  // 255 = full RGB
...
CHSV constrain_palette(uint8_t i, CHSV color) {
  ...
  color.h = map8(sin8(color.h), palette_start, palette_start + palette_width);
  return color;
}
...
{ 
  if (c1 == c2) {
    ...
    leds[i] = constrain_palette(i, CHSV(interpolate_wrap(c1.h, c2.h, fract), interpolate(c1.s, c2.s, fract), interpolate(c1.v, c2.v, fract)));
  }
}
