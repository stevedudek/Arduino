...
void test_pentagon_type(uint8_t channel) {
  shows[channel].fillBlack();
  
  for (uint8_t x = 0; x < SIZE; x++) {
    for (uint8_t y = 0; y < SIZE; y++) {
      shows[channel].setPixeltoHue(get_pixel_from_coord(x, y), get_pentagon_type(x, y) * 50);
    }
  }
}
...
//
// Get Pentagon Type: 0-3
//
uint8_t get_pentagon_type(uint8_t x, uint8_t y) {
  return (x % 2) + ((y % 2) * 2);
}
...
void center_ring(uint8_t c) {
  ring(c, shows[c].getForeColor(), shows[c].getForeBlack(), 128, 128, 8, 64, 128);
}
...
void corner_ring(uint8_t c) {
  ring(c, shows[c].getForeColor(), shows[c].getForeBlack(), 0, 0, 32, 64, 255);
}
...
void get_coord_position(uint8_t x, uint8_t y, uint8_t *x_pos, uint8_t *y_pos) {
  uint8_t pentagon_type = get_pentagon_type(x, y);
