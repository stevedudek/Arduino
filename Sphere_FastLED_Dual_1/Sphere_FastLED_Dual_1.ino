...
//  12/14/2018
//
#define NUM_LEDS 11  // Virtual LEDS (actual = 12 * 6 = 72 for the hemisphere)
#define NUM_FACES 6   // For the hemisphere
...
//
// Deconvolve the 11 LEDs for one face on to all 6 faces
//
void morph_channels(uint8_t fract) {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    for (uint8_t face = 0; face < 6; face++) {                            
      leds[(face * 12) + i] = color;
      if (i == 10) {
        leds[(face * 12) + 11] = color;  // Make identical the top + bottom hub pixels
      }
    }
  }
}
