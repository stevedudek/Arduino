...
//  FastLED
//
//  xBee - Listening Mode - Disabled self-changing parameters
//
...
const uint8_t pattern_matrix[][8] = {
  ...
  { 0xf2, 0x19, 0xdf, 0x3e, 0xf9, 0xee, 0x68, 0x0 },
  ...
  { 0xf7, 0x5b, 0x8c, 0xc, 0x76, 0xb9, 0xbf, 0xf8 },
...
const uint8_t colored_pattern_matrix[][16] = {
  ...
  { 0xaa, 0x1b, 0xe, 0x40, 0x6, 0xce, 0x40, 0x1, 0xb0, 0xe4, 0x6, 0xc3, 0x9b, 0xf, 0xa5, 0x0 },
...
//    change_it_up();  // Disabled for xBee
  }
...
void next_show() {
  ...
  */
  ...
  small_cycle = 0;
  cycle = 0;
//  wait = random8(NUM_WAIT_VALUES);  // Disabled for xBee
//  total_frames = getNumFrames(wait);  // Disabled for xBee
  set_noise_parameters();
  set_all_black();
//  foreColorSpeed = up_or_down(foreColorSpeed, MIN_COLOR_SPEED, MAX_COLOR_SPEED);  // Disabled for xBee
//  backColorSpeed = up_or_down(backColorSpeed, MIN_COLOR_SPEED, MAX_COLOR_SPEED);  // Disabled for xBee
}
...
void HearSignal() {
  if (Serial.available()) {  // Heard a signal!
      ...
      for (uint8_t i = 0; i < MAX_MESSAGE; i++) { message[i] = 0; }  // Clear message
...
void GetMessage(char* message) {
  ...
  while (i < MAX_MESSAGE) {
    if (!Serial.available()) {
      message[i] = COMMAND_PERIOD;
      return;  // Can't hear any more
    }
...
boolean ProcessMessage(char* message) {
  ...
  while (i < MAX_MESSAGE) {
    ...
    char number[MAX_NUM];
    for (uint8_t j = 0; j < MAX_NUM; j++) { number[j] = 0; }  // Clear number buffer
    
    int numsiz = ReadNum(message, i+1, number);  // Read in the number
    i = i + 2 + numsiz; // 2 = leap beginning command and trailing comma
    
    ExecuteOrder(command, atoi(number));
  }
}
...
uint8_t ReadNum(char* msg, uint8_t place, char* number) {
  ...
  while (i < MAX_NUM) {
    tmp = msg[place];
    if (tmp == COMMAND_COMMA || tmp == COMMAND_PERIOD) {
      return (i);
    } else {
      number[i] = msg[place];
      i++;
...
//
void ExecuteOrder(char command, int value) {
...
//    color = CHSV(c2.h, c2.s, interpolate(c1.v, c2.v, fract));  // safe approach
    color = CHSV(c2.h, c2.s, dim8_lin(scale8(c2.v, fract))); // try this too
  } else if (is_black(c2)) {
//    color = CHSV(c1.h, c1.s, interpolate(c1.v, c2.v, fract));  // safe approach
    color = CHSV(c1.h, c1.s, dim8_lin(scale8(c1.v, 255 - fract))); // try this too
  } else {
