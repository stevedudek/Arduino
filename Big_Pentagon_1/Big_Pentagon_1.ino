...
uint8_t DELAY_TIME = 60;  // in milliseconds. A cycle takes 3-5 milliseconds. 15+ is safe.
long last_time;
...
void patterns(uint8_t c) {
  for (uint8_t x = 0; x < SIZE; x++) {
    for (uint8_t y = 0; y < SIZE; y++) {
      boolean value = get_bit_from_pattern_number(((y % 6) * 6) + (x % 6), show_variables[c]);
...
void center_ring(uint8_t c) {
  ring(c, shows[c].getForeColor(), BLACK, 128, 128, 8, 64, 128);
}

void well(uint8_t c) {
  ring(c, shows[c].getForeColor(), led[c].wheel(shows[c].getBackColor()), 128, 128, 16, 192, 192);
}

void corner_ring(uint8_t c) {
  ring(c, shows[c].getForeColor(), BLACK, 0, 0, 32, 64, 255);
}
...
CHSV mask(CHSV color, uint8_t i, uint8_t channel) {
  ...
  if (value) {
    return (mask_value > 2) ? BLACK : led[channel].wheel(shows[channel].getBackColor());
  } else {
    return color;
  }
}
