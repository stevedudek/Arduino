...
#define STEM_LEDS 76
#define TOTAL_LEDS  (2 * ((ONE_TREE_LEDS * 2) + STEM_LEDS))  // Symmetric - 2 Y's + 1 Stem
#define ONE_BRANCH (TOTAL_LEDS / 2)
...
void morph_channels(uint8_t fract) {
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    leds[n] = color;
    leds[n + ONE_BRANCH] = color;
    if (n > 38) {
      leds[n + ONE_TREE_LEDS] = color;
      leds[n + ONE_TREE_LEDS + ONE_BRANCH] = color;
    }
