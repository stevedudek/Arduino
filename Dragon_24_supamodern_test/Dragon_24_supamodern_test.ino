  ...
  for (uint8_t section = 0; section < NUM_SECTIONS; section++) {
    ...
    for (uint8_t column = 0; column < get_section_table_value(section, SECT_NUM_COLUMNS_FIELD); column++) {
      ...
      for (uint8_t scale = 0; scale < column_height; scale++) {
        ...
        if (morph_channels_for_pixel(scale_number, adj_led_number, fract)) {
...
bool morph_channels_for_pixel(uint16_t scale_number, uint16_t led_number, uint8_t fract) {
  CRGB rgb_color;
  ...
  if (led_buffer[scale_number].h == color.h && led_buffer[scale_number].s == color.s && led_buffer[scale_number].v == color.v) {
    return false;
  } else {
...
//
uint8_t get_frame_brightness(uint8_t c) {
  CHSV color;
  uint32_t value_total = 0;
  
  for (uint16_t i = 0; i < NUM_SCALES; i++) {
    color = led[c].getCurrFrameColor(i);
    value_total += color.v;
  }
  return (value_total / NUM_SCALES);  // average scale value for the frame
}

void add_show_brightness() {
  for (uint8_t i = 0; i < DUAL; i++) {
    show_brightness[i] += get_frame_brightness(i);
  }
}
