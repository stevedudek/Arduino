...
//
#define SNOWFLAKE_NUMBER 0  // 0 = Small Simple, 1 = Medium Pointy, 2 = Large Complex, 3 = Medium Inward
...
// Shows
#define START_SHOW_CHANNEL_A  8  // Starting show for Channels A
#define START_SHOW_CHANNEL_B  0  // Starting show for Channels B
...
const int8_t shift_matrix[] = { -1, -1, -1, -1 };  // ToDo: Figure out the shift numbers (see notes above)
...
void pick_symmetry(uint8_t i) {
  ...
  switch_axis[i] = (random8(2) == 0 && current_show[i] != 0) ? true : false;
  set_symmetry(i);
}
  ...
  for (uint8_t i = 0; i < pitch; i++) {
    for (uint8_t j = 0; j < symmetries[symmetry[channel]]; j++) {
      ...
      if (SNOWFLAKE_NUMBER == 1 && switch_axis[channel]) {
        trans_i = ((i % 6) * HUB_LENGTH) + (i / 6);  // ToDo: Check this!
      }
...
void draw_spokes(uint8_t channel) {
  ...
  if (symm_type >= 4) {
    draw_spokes_symm_4(channel);  // Special high-symmetry case. Ignore everything else.
    return;
  }
  ...
  for (i = 0; i < arm_length; i++) {
    ...
    for (uint8_t j = 0; j < symm; j++) {  // 1, 2, 3, 6, 6
      ...
      pixel = shift_pixel(pixel, SNOWFLAKE_NUMBER);  // shift pixel, if necessary, by snowflake type
...
void draw_spokes_symm_4(uint8_t channel) {
  ...
  for (uint8_t i = 0; i < (SPOKE_LENGTH / 2) + 1; i++) {
    color = led[channel].getInterpFrameColor(get_hub_pixels(symmetry[channel]) + i);
    color = dim_spoke(color);
    
    for (uint8_t spoke = 0; spoke < 6; spoke++) {
      pixel = (SPOKE_LENGTH - i) + (spoke * SPOKE_LENGTH);
      pixel = shift_pixel(pixel, SNOWFLAKE_NUMBER);  // ToDo: Check shift_pixel
      leds_buffer[channel][pixel] = color;  // Draw left side
      
      pixel = i + (spoke * SPOKE_LENGTH);
      pixel = shift_pixel(pixel, SNOWFLAKE_NUMBER);  // ToDo: Check shift_pixel
      leds_buffer[channel][pixel] = color;  // Draw right side
    }
  }
}
...
//
uint8_t shift_pixel(uint8_t i, uint8_t snowflake_type) {
  return (i + SPOKE_LEDS + shift_matrix[snowflake_type]) % SPOKE_LEDS;
}
