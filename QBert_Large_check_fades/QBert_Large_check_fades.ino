...
//  9/14/21
//
//  Listens to the Network: will this sketch run out of memory?
...
#define DATA_PIN 13  // 0  // 11
#define CLOCK_PIN 14  // 2  // 13
...
//  FastLED.addLeds<WS2801, DATA_PIN, CLOCK_PIN>(leds, TOTAL_LEDS);
  // Speeds: < 50 KHz may not be allowed by the compiler; > 10 MHz freaks out
  // Don't see much change in flickering with data rate; try a different fix
  // Although 50 KHz (and 100 KHz) both look not bad
  // 1. Look at pins. Get on to clock pins.
  // 2. Try with and without the level converter
  // 3. See whether this is a software problem: make a simple program that slowly changes color
  FastLED.addLeds<WS2801, DATA_PIN, CLOCK_PIN, RGB>(leds, TOTAL_LEDS);
//  FastLED.addLeds<WS2801, DATA_PIN, CLOCK_PIN, RGB, DATA_RATE_MHZ(1)>(leds, TOTAL_LEDS);
...
//  Serial.printf("%d, ", ease8InOutApprox(intensity));
