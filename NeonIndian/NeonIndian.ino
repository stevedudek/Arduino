...
uint32_t currColor  = 85;   // Foreground color
uint32_t currColor2 = 170;  // Background color
...
void loop() {
 ...
 key = KeyPad();
  ...
  if (key == DOWN || !random(5000)) { // Show needs to advance
  ...
  switch (show) {
    ...
    case 0: {  // Center Feather Only - Randomly do lights run down
      
      if(key == CENTER) colorWipe(Wheel(currColor), 20);       // CENTER = turn on all lights
      if(key == LEFT || !random(50)) oneLightRunDown(Wheel(currColor), 40);   // LEFT = one light run down
      if(key == RIGHT) oneLightRunUp(Wheel(currColor), 40);    // RIGHT = one light run up
      if(key == UP || !random(3)) {    // UP = change color
        if (currColor++ >= MaxColor) currColor = 0;
      }
      // Neither center nor up? Then turn only the center light on
      if(key != CENTER) LightOneLight(numLights/2, currColor, 250);
      break;
    }
    ...
    case 1: {   // Light running show
      ...
      if (key == LEFT || !random(5)) {       // LEFT = change foreground color
        if (currColor++ >= MaxColor) currColor = 0;
      }
      if (key == RIGHT || !random(3)) { // RIGHT = change background color
        if (currColor2++ >= MaxColor) currColor2 = 0;
      }
      LightRunUp(f, x, Wheel(currColor), Wheel(currColor2), GetDelayTime(wait)); // post frame of show
      if (f++ >= wingLights) {
        f=0;      // advance frame. Set to 0 if at end
        if (x++>wingLights) x=1;  // add another running light
      }
      break;
    ...
    case 2: {  // Rainbow show
      ...
      if(key == UP || !random(500)) { x++; }  // UP = increase color divisions
      break;
    ...
    case 3: {   // Lightwaves show
      ...
      if (key == CENTER) { colorWipe(Wheel(currColor), 20); }  // CENTER = turn on all lights
      if (key == UP || !random(6)) {       // UP = change color
        if (currColor++ >= MaxColor) currColor = 0;
      }
      if (key == LEFT) { // LEFT = slow speed
        if (wait++ >= maxWait) wait=0;
      }
      if (key == RIGHT || !random(9)) { // RIGHT = change background color
        if (currColor2++ >= MaxColor) currColor2 = 0;
      }
  ...
  case 5: {  // Color Gradient
      ...
      if(key == UP || !random(6)) {          // UP = change start of gradient
        if (currColor++ >= MaxColor) currColor = 0;
      }
