...
uint8_t BRIGHTNESS = 255;  // (0-255) (ArduinoBlue)  CHANGEBACK
...
//  Serial2.begin(9600);  // Serial2: Bluetooth serial (ArduinoBlue)
  ...
  for (uint8_t i = 0; i < DUAL; i++) {
    ...
    shows[i].setColorSpeedMinMax(10, 30);  // Make colors change faster (lower = faster)
...
void pendulum_wave(uint8_t i, boolean smoothed) {
  ...
  if (shows[i].isShowStart()) {
    freq_storage[i] = random(35, 45);
    shows[i].turnOffMorphing();
  }
  
  for (uint8_t x = 0; x < SIZE; x++) {
    value = beatsin8(freq_storage[i] - (1 * x));
    for (uint8_t y = 0; y < SIZE; y++) {
...
void game_of_life(uint8_t i, boolean faster) {
  if (shows[i].isShowStart() || count_life() < 20) {
