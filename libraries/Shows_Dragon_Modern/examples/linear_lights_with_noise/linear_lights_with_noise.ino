...
Noise noise = Noise(NUM_LEDS, MAX_HUE_NOISE, MAX_SAT_NOISE);  // library class object of Noise.h
...
void noisyshow() {
  ...
  for (int i=0; i < NUM_LEDS; i++) {
    setPixelColor(i, CHSV(noise.getRawNoiseA(i), 255, noise.getRawNoiseB(i) ));
  }
}
...
CHSV constrain_palette(uint8_t i, CHSV color) {
  if (noise.isNoiseOn()) {
    color.s = noise.addNoiseAtoValueNoWrap(i, color.s);
    color.h = noise.addNoiseBtoValue(i, color.h);
  }
