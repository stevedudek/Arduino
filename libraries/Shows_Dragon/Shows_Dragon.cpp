...
{
  // wrapper for led.morph_frame()
  uint8_t morphFract = (has_morphing) ? getMorphFract() : 255;
//  if (channel == 0) {
//    Serial.printf("%d, %d, %d\n", morphFract, getSmallCycle(), get_intensity());
//  }
  led->morph_frame(morphFract);
}

uint8_t Shows::getMorphFract(void) {
  // Calculate the 0-255 interpolation fraction
  uint16_t curr_cycle_time = getElapsedCycleTime();
  uint8_t morph_fract = (curr_cycle_time > cycle_duration) ? 255 : 255 * curr_cycle_time / cycle_duration;
  return  morph_fract;
}
...
{
  start_show_time = millis() + (show_duration * 1500);
}
...
{
  return millis() > getFinishShowTime();
}
...
{
  cycle_duration = max(cycle_duration / r, delay_time * 2);
}

void Shows::makeWaitSlower(uint8_t r)
{
  cycle_duration = min(cycle_duration * r, 2000);
}
...
void Shows::packets_two(void)
{
  ...
  for (uint8_t i=0; i < max_packets; i++) {
    ...
    // Apply each packet to the lights
    for (uint16_t j=0; j < numLeds; j++) {
      wave = beatsin8(packet_freq, 0, 255, 0, j % 255);
...
void Shows::patterns(void)
{
  // Reset a lot of variables at the start of the show
  if (isShowStart()) {
    Serial.println("Show is starting");
