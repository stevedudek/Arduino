...
void morph_channels(uint8_t fract) {
  for (int i = 0; i < NUM_LEDS; i++) {
    // RGB interpolation
    leds[CHANNEL_A][i] = led[CHANNEL_A].getInterpRGB(leds[CHANNEL_B][i], leds[CHANNEL_A][i], fract);  // interpolate a + b channels

    /*
    // a + b RGBs -> HSVs -> Interpolate -> HSV -> push on to Channel A (255 = all A)
    leds[CHANNEL_A][i] = led[CHANNEL_A].getInterpRGBthruHSV(leds[CHANNEL_B][i], leds[CHANNEL_A][i], fract);
    */
  }
}
