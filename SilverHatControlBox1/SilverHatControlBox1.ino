...
// Light tails
//
// turn on the tail lights to a gradient
//
...
void lighttails() {
  ...
  for (i=0; i < strip.numPixels(); i++) {
    if (!Tails[i]) setPixelColor(SpineOrder[i], Color(0,0,0)); // Black
    else setPixelColor(SpineOrder[i], Wheel(colors[Tails[i]-1]));
  }
}
...
// Light hood
//
// ring the hood with one color; fill everything else with another color
//
...
void lighthood() {
  ...
  // Assign spines to the proper color
  for (i=0; i < strip.numPixels(); i++) {
    if (!Hood[i]) setPixelColor(SpineOrder[i], Color(0,0,0)); // Black
    else setPixelColor(SpineOrder[i], Wheel(colors[Hood[i]-1]));
  }
}
...
// Center gradient
//
// turn on the central spine with a light gradient
//
...
void centergradient() {
  ...
  byte colors[12];
  byte tempcolor = backColor;
  byte colorspacing = foreColor / 12;
  ...
  // Assign spines to the proper color
  for (i=0; i < strip.numPixels(); i++) {
    if (!Stripes[i]) setPixelColor(SpineOrder[i], Color(0,0,0)); // Black
    else setPixelColor(SpineOrder[i], Wheel(colors[Stripes[i]-1]));
  }
}
...
// Center Pulse
//
// One pixel travelling down the center spine
//
void centerpulse() {
  for (int i = 0; i < numLights; i++) {
    if (Stripes[i] == cycle % 12) {
