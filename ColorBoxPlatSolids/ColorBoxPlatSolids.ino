...
void CheckBox() {
  ...
  if (abs(dial - dialval[0]) > MIN_CHANGE) { // Has dial #0 budged?
    ...
    foreColor = dial * 3;   // Set new color. 0-1024 on dial into 0-3072 for color
    message = true;
  }
  
  // Dial #1 - The backColor dial
  dial = GetDial(1);
  if (abs(dial - dialval[1]) > MIN_CHANGE) { // Has dial #1 budged?
    dialval[1] = dial;      // Update dial
    backColor = dial * 3;   // Set new color. 0-1024 on dial into 0-3072 for color
    message = true;
  }
