...
void pick_symmetry(uint8_t i) {
  ...
  switch_axis[i] = (random8(2) == 0) ? true : false;
  set_symmetry(i);
}
...
void draw_spokes(uint8_t channel) {
  ...
  uint8_t symm = symmetries[symm_type];  // Number of arms: 1, 2, 3, 6, 6
  ...
  if (symm_type < 4) {
    Serial.println(pitch / SPOKE_LENGTH);
    for (uint8_t i = 0; i < pitch; i++) {  // SPOKE_LEDS / symm;  
      for (uint8_t j = 0; j < symm; j++) {  // 1, 2, 3, 6, 6
        trans_i = (switch_axis[channel]) ? transform_axis(i, pitch / SPOKE_LENGTH) : i ;
...
// switch_axis
//
uint8_t transform_axis(uint8_t i, uint8_t num_arms) {
  uint8_t i_offset, trans_i;
  
  i_offset = i / (2 * num_arms);
  if (i % 2 == 1) {
    i_offset = SPOKE_LENGTH - i_offset - 1;
  }
  if (i_offset == SPOKE_LENGTH / 2) {  // Is this an outer tip?
    trans_i = (i % num_arms) * SPOKE_LENGTH;  // yes
  } else {
    trans_i = ((i / 2) % num_arms) * SPOKE_LENGTH;
  }
  return(i_offset + trans_i);
}
