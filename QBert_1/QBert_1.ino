  ...
  for (uint8_t i = 0; i < DUAL; i++) {
    ...
    switch (current_show[i]) {
      ...
      case 3:
        ...
        break;
      case 4:
        shows[i].plinko(0);  // ToDo: FIX
        break;
      case 5:
        windmill(i);  // ToDo: FIX
        break;
      case 6:
        windmill_smoothed(i);  // ToDo: FIX
        break;
      case 7:
        cone_push(i);
        break;
...
void cone_push(uint8_t i) {
  ...
  for (uint8_t x = 0; x < SIZE; x++) {
    for (uint8_t y = 0; y < SIZE - x; y++) {
      for (uint8_t z = 0; z < DIAMOND_NUMBER; z++) {
        uint8_t value = 255 - (abs(beat - get_coord_distance_to_origin(x,y,z)) * 2);
...
uint8_t get_coord_distance_to_origin(uint8_t x, uint8_t y, uint8_t z) {
  return get_coord_distance_to_point(x, y, z, 128, 128);
}
...
uint8_t get_cartesian_x(uint8_t x, uint8_t y, uint8_t z) {
  float z_lookup_delta_x[] = { -0.25, 0.25, 0 };
  float cartesian_x = 70 * (x + (float(y) * 0.5) + 0.25);
  cartesian_x += (70 * z_lookup_delta_x[z]);
...
uint8_t get_cartesian_y(uint8_t x, uint8_t y, uint8_t z) {
  float z_lookup_delta_y[] = { 0.144, 0.144, -0.289 };
  float cartesian_y = ((0.866 * float(y)) + 0.5);
  cartesian_y += (70 * z_lookup_delta_y[z]);
...
void game_of_life(uint8_t i, boolean faster) {
  if (shows[i].isShowStart() || count_life() < 2) {
...
uint8_t get_pixel_from_coord(uint8_t x, uint8_t y, uint8_t z) {
  ...
  return ((x + SPACING[y]) * DIAMOND_NUMBER) + z;
}
