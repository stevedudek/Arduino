...
//// TRYING to debug 3rd Snowflake. This is a scratch file only.
// Built off of Snowflake_Big_Dual_7
...
#define DELAY_TIME 80 // in milliseconds (usually 40)
...
// Shows
#define START_SHOW_CHANNEL_A  0  // lightRunUp Starting show for Channels A
#define START_SHOW_CHANNEL_B  1  // fillBlack Starting show for Channels B
uint8_t current_show[] = { START_SHOW_CHANNEL_A, START_SHOW_CHANNEL_B };
uint8_t current_pattern[] = { 5, 0 };
boolean has_black_pattern[] = { true, true };
#define NUM_SHOWS 14
#define SINGLE_SPOKE_SHOWS 3  // Shows here and above can have a single spoke

// Clocks and time
#define SHOW_DURATION 300  // seconds. Size problems at 1800+ seconds.
  ...
  for (uint8_t i = 0; i < pitch; i++) {
    for (uint8_t j = 0; j < symmetries[symmetry[channel]]; j++) {
      ...
      if (switch_axis[channel]) {
        pixel = HUB_LEDS - 1 - pixel + (2 * SPOKE_LEDS);  // Invert the hub to get orientation correct
      }
  ...
  if (symm < 4) {
    ...
    for (uint8_t i = 0; i < pitch; i++) {      
      for (uint8_t j = 0; j < symmetries[symm]; j++) {
        ...
        leds_buffer[channel][pixel] = led[channel].getInterpFrameColor(get_hub_pixels(symm) + i);
      }
    }
  } else {  // For 12-fold symmetry
    for (uint8_t i = 0; i < ((SPOKE_LENGTH/2)+1); i++) {
      for (uint8_t spoke = 0; spoke < 6; spoke++) {
        leds_buffer[channel][shift_one((SPOKE_LENGTH - i) + (spoke * SPOKE_LENGTH))] = led[channel].getInterpFrameColor(get_hub_pixels(symm) + i);
        leds_buffer[channel][shift_one(i + (spoke * SPOKE_LENGTH))] = led[channel].getInterpFrameColor(get_hub_pixels(symm) + i);        
      }
    }
  }
}
