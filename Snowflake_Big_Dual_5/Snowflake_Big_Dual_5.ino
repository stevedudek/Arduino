...
//
//  Try to save on memory. ALGORITHM WORKS BUT BLACKENS PIXELS (InterpFrame switcheroo not correct).
//
//  Removed: Noise, Shuffle shows, rotating, xBee (restore later?)
//
//  New, new, new algorithm:
//
//  Push around content with the 2 InterFrames (order of operation is key)
//  Interpolate the 2 InterFrames on to the CRGB leds
//  Least amount of memory consumption, but pixels may get weirdly scrambled
//
//  ToDo: Either 
//  1. Get boards with more memory (USE Snowflake_Big_Dual_4 with Teensy)
//  2. Use the RGB-morphing (USE Snowflake_Big_Dual_3 but accept RGB morphing)
//  3. Rewrite below NOT to use CHSV leds_buffer[DUAL][NUM_LEDS] (USE Snowflake_Big_Dual_5 here but lose pixels)
//
...
const uint8_t hub_pixels[] = { 7, 4, 3, 2, 2 };  // Don't change: 1 + (6 / symmetries[n]);
...
void loop() {
  ...
  interpolate_leds();  // morph together the 2 leds channels and deposit on to Channel_A
...
//
// interpolate_leds - interpolate the two led[channel].interpframes and push result on to leds
//
void interpolate_leds() {
  ...
  if (intensity_a == 0) {
    for (int i = 0; i < NUM_LEDS; i++) {  // Channel A is off. Push Channel B.
      leds[i] = led[CHANNEL_B].getInterpFrameColor(i);
    }
  } else if (intensity_a == 255) {
    for (int i = 0; i < NUM_LEDS; i++) {  // Channel B is off. Push Channel A.
      leds[i] = led[CHANNEL_A].getInterpFrameColor(i);
    }
  } else {
...
void update_leds() {
  for (uint8_t i = 0; i < DUAL; i++) {
    draw_center(i);  // Order of operations is important
    draw_hub(i);
    draw_spokes(i);
  }
}
...
void draw_hub(uint8_t channel) {
  uint8_t pixel;
  uint8_t pitch = 6 / symmetries[symmetry[channel]];
  for (uint8_t i = 0; i < pitch; i++) {  // Order of operations is important
    for (uint8_t j = 0; j < symmetries[symmetry[channel]]; j++) {
      ...
      // May scramble the InterpFrame
      led[channel].setInterpFrame(pixel, led[channel].getInterpFrameColor(1 + i));
    }
  }
}
  ...
  if (symm < 4) {
    ...
    for (uint8_t i = 0; i < pitch; i++) {  // Order of operations is important   
      for (uint8_t j = 0; j < symmetries[symm]; j++) {
        ...
        // May scramble the InterpFrame
        led[channel].setInterpFrame(pixel, led[channel].getInterpFrameColor(hub_pixels[symm] + i));
      }
    }
    ...
    for (uint8_t i = 0; i < ((SPOKE_LENGTH/2)+1); i++) {
      for (uint8_t spoke = 0; spoke < 6; spoke++) {
        // May scramble the InterpFrame
        led[channel].setInterpFrame(shift_one((SPOKE_LENGTH - i) + (spoke * SPOKE_LENGTH)), led[channel].getInterpFrameColor(hub_pixels[symm] + i));
        led[channel].setInterpFrame(shift_one(i + (spoke * SPOKE_LENGTH)), led[channel].getInterpFrameColor(hub_pixels[symm] + i));
      }
    }
  }
}
