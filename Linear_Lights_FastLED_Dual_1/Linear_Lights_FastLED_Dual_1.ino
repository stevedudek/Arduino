...
//  10/9/2018
//
#define NUM_LEDS 100  // Chance of memory shortage for large NUM_LEDS
...
Shows shows[] = { Shows(&led[0]), Shows(&led[1]) };  // 2 Show libraries
...
// Shows
#define NUM_SHOWS 11
uint8_t current_show[] = { 0, 1 };   // Startings shows for Channels A + B
...
// noise - applies to combined shows
#define HAVE_NOISE false    // set to false to suppress noise
Noise noise = (HAVE_NOISE) ? Noise(NUM_LEDS) : Noise() ;
...
void setup() {
  ...
  if (HAVE_NOISE) {  // Noise will apply to combined shows
    noise.turnNoiseOn();
  } else {
    noise.turnNoiseOff();
  }
...
//
void add_noise(uint8_t channel) {
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    led[channel].setInterpFrameHue(i, noise.addNoiseAtoValue(i, led[channel].getInterpFrameHue(i))); 
    led[channel].setInterpFrameSat(i, noise.addNoiseBtoValueNoWrap(i, led[channel].getInterpFrameSat(i)));
  }
}
