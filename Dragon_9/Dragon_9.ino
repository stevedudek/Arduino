...
uint8_t section_list[] = { SECTION_HEAD, SECTION_A };
...
#define NUM_SCALES   (86 + 86)  // set this manually
#define NUM_SPACERS  (17 + 21)  // set this manually
...
const int8_t head_column_table[] PROGMEM = {
  1, 0, 5, 0,   //  0
  1, 0, 6, 5,   //  1
  1, 0, 6, 11,  //  2
  2, 1, 7, 17,  //  3
...
void test_neighs(uint8_t c) {
  ...
  for (int8_t x = min_x; x < max_x; x++) {
    for (int8_t y = min_y; y < max_y; y++) {
      uint16_t coord = get_coord_from_xy(x, y);
      uint16_t scale = get_scale_for_coord(coord);

      if (scale != XX && scale % 20 == 0) {
        shows[c].setPixeltoForeColor(scale);  // Draw the center
        for (uint8_t dir = 0 ; dir < 6; dir++ ) {
          shows[c].setPixeltoBackColor(get_scale_for_coord(get_coord_in_dir(dir, coord)));
        }
      }
    }
  }
