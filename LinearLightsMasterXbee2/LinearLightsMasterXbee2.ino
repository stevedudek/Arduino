...
int show = 1;       // Starting show. Dark = 0.
...
void loop() { 
  ...
  delay(20);  // Important delay. No talkdelay. 
  ...
  if (!random(20000)) {
    // toggle show between dark and light
    if (show) {
      show = 0;
    } else {
      show = random(MAX_SHOW-1) + 1;
    }
  ...
  if (!random(1000)) {
    ...
    SendMessage(COMMAND_DELAY, wait);
  }
}
