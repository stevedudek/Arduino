...
byte ConeSize[45] = { 5,5,5,3,5,1,3,5, 1,1,5,4, 3,4,5,5,6,3,6,2,1,6,6,4, 2,6,2,1,3,4, 2,4,4,5,3,2, 2,3,1, 6, 3,3, 1};
...
void sendPixelColor(byte pos, byte color) {
  byte cone = ConeLookUp[pos];
  
  switch(color) {
    case BLACK:
      strip[cone] = CRGB::Black;
      break;
    case WHITE:
      strip[cone] = CRGB::White;
      break;
    default:
      strip[cone].setHue(color); 
      break;
  }
}
