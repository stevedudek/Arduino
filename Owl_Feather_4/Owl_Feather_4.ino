...
//
// get pixel for xy (column, row) - return the scale number (or XX if out of bounds)
//
uint8_t get_pixel_from_xy(int8_t x, int8_t y) {
  return get_pixel_for_coord(get_coord_from_xy(x, y));
}


uint8_t get_pixel_for_coord(uint16_t coord) {
...
//        Serial.printf("section, column, row (%d, %d, %d)\n", section, column, row);
    ...
    if (y >= y_start && row < y_start + get_column_table_value(x, COLUMN_HEIGHT_FIELD)) {
      uint8_t pixel = (y - y_start) + get_column_table_value(x, COLUMN_PIXEL_START_FIELD);
...
//
// Coord lighting functions
//
void setCoordtoHue(uint16_t coord, uint8_t hue, uint8_t c) {
  shows[c].setPixeltoHue(get_pixel_for_coord(coord), hue);
}

void setCoordtoColor(uint16_t coord, CHSV color, uint8_t c) {
  shows[c].setPixeltoColor(get_pixel_for_coord(coord), color);
}

void addColortoCoord(uint16_t coord, CHSV color, uint8_t c) {
  led[c].addPixelColor(get_pixel_for_coord(coord), color);
}

void flipPixel(uint8_t i, uint8_t c) {
  if (led[c].getNextFrameColor(i).v == 0) {
...
bool is_coord_inbounds(uint16_t coord) {
  int8_t x = get_x_from_coord(coord);
  int8_t y = get_y_from_coord(coord);
  return (x >= 0 && x < MAX_HEIGHT && y >= 0 && y < MAX_COLUMNS);
}

uint16_t get_coord_col_min_y(int8_t x) {
  for (int8_t y = 0; y < MAX_COLUMNS; y++) {
...
uint16_t get_coord_col_max_y(int8_t x) {
  for (int8_t y = MAX_COLUMNS; y >= 0; y--) {
    if (get_scale_from_xy(x, y) != XX) {
      return get_coord_from_xy(x, y);
    }
  }
