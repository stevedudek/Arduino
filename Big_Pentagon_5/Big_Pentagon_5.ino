...
//
//  Small Pentagon: 12 x 12 = 144 lights in a square/pentagon grid
//
//  9/5/20
//
...
uint8_t show_variables[] = { 0, 0, 0, 0, 0, 0, 0, 0 };  // 2 each: pattern, rotate, symmetry, mask

// wait times
#define SHOW_DURATION 120  // seconds
uint8_t FADE_TIME = 60;  // seconds to fade in + out (Arduino Blue)
...
void pick_next_show(uint8_t i) {
  ...
  uint8_t symmetry = 0; 
  uint8_t rotation = 0;
  current_show[i] = is_other_channel_show_zero(i) ? random(2, NUM_SHOWS) : random(0, 2) ;
...
boolean is_other_channel_show_zero(uint8_t channel) {
  if (channel == 0) {
    return (current_show[CHANNEL_B] <= 1);
  } else {
    return (current_show[CHANNEL_A] <= 1);
  }
}
...
void patterns(uint8_t c) {
  for (uint8_t x = 0; x < SIZE; x++) {
    for (uint8_t y = 0; y < SIZE; y++) {
      boolean value = get_bit_from_pattern_number((y * SIZE) + x, show_variables[c]);
...
//
// color patterns shows
//
void color_patterns(uint8_t c) {
  if (shows[c].isShowStart()) {
    show_variables[c] = random(NUM_COLOR_PATTERNS * 2);
  }
  uint8_t cycle = shows[c].getCycle();
  uint8_t color_pattern_number = show_variables[c] / 2;
  uint8_t max_pattern_lookup[] = { 15, 9, 8, 15, 8, 15 };
  uint8_t max_pattern = max_pattern_lookup[color_pattern_number];
  
  for (uint8_t x = 0; x < SIZE; x++) {
    for (uint8_t y = 0; y < SIZE; y++) {
      uint8_t value = get_short_from_color_pattern((y * SIZE) + x, color_pattern_number);
      uint8_t i = get_pixel_from_coord(x,y);

      if (value == 0) {
        if (show_variables[c] >= NUM_COLOR_PATTERNS) {
          shows[c].setPixeltoForeColor(i);
        } else {
          shows[c].setPixeltoForeBlack(i);
        }
      } else {
        uint8_t amount = map(abs((cycle % max_pattern) - value), 0, max_pattern, 0, 255 );
        if (show_variables[c] % 2 == 0) {
          shows[c].setPixeltoHue(i, shows[c].IncColor(shows[c].getBackColor(), amount / 5) );
        } else {
          led[c].setPixelColor(i, CHSV(shows[c].getBackColor(), 255, dim8_raw(amount)) );
        }
      }
    }
  }
...
void test_interpolation() {
  uint8_t hue1, hue2, hue_interp, fract;
  CHSV color;
  
  for (hue1 = 0; hue1 < MAX_COLOR; hue1++) {
    for (hue2 = 0; hue2 < MAX_COLOR; hue2++) {
      hue_interp = hue1;
      for (fract = 0; fract < MAX_COLOR; fract++) {
        CHSV color = led[CHANNEL_A].getInterpHSV(CHSV(hue1, 255, 255), CHSV(hue2, 255, 255), fract);
        if (abs(hue_interp - color.h) > 1) {
          Serial.print("interpolation error: ");
          Serial.print(hue1);
          Serial.print(", ");
          Serial.print(hue2);
          Serial.print(": ");
  ...
  Serial.println("Finished interpolation test");
}
...
void cone_push(uint8_t i) {
  float center_x = (SIZE - 1) / 2.0;
  float center_y = (SIZE - 1) / 2.0;
  ...
  for (uint8_t x = 0; x < SIZE; x++) {
    for (uint8_t y = 0; y < SIZE; y++) {
      ...
      led[i].setPixelColor(get_pixel_from_coord(x,y), CHSV(shows[i].getForeColor(), 255, dim8_raw(value)) );
    }
  }
}
...
void corner_ring(uint8_t c) {
  ring(c, shows[c].getForeColor(), shows[c].getForeBlack(), 0, 0, 3, 64, 255);
}
...
boolean get_bit_from_pattern_number(uint8_t n, uint8_t pattern_number) {
  uint8_t PatternMatrix[] = {
    ...
    0x99, 0x96, 0x66, 0x66, 0x69, 0x99, 0x99, 0x96, 0x66, 0x66, 0x69, 0x99, 0x19, 0x96, 0x66, 0x66, 0x69, 0x98,
    0x0, 0x6, 0x0, 0x60, 0xc1, 0x8c, 0x18, 0x0, 0x60, 0x6, 0x0, 0x18, 0x31, 0x83, 0x6, 0x0, 0x60, 0x0,
    0x0, 0x0, 0x60, 0x9, 0x1, 0x8, 0x20, 0x44, 0x62, 0x46, 0x22, 0x4, 0x10, 0x80, 0x90, 0x6, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0xc, 0x3, 0xc0, 0x33, 0xc, 0x30, 0xcc, 0x3, 0xc0, 0x30, 0x0, 0x0, 0x0, 0x0,
    ...
    0x0, 0x80, 0x66, 0x6, 0x61, 0x81, 0x18, 0x6, 0x6, 0x60, 0x60, 0x18, 0x81, 0x86, 0x60, 0x66, 0x1, 0x0,
    0x20, 0x3, 0x0, 0xc0, 0x4, 0x60, 0x6, 0x1, 0xf8, 0x1f, 0x80, 0x60, 0x6, 0x20, 0x3, 0x0, 0xc0, 0x4,
    ...
    0x60, 0xe, 0x0, 0xe0, 0x1, 0xc0, 0x18, 0x1, 0x0, 0x0, 0x80, 0x18, 0x3, 0x80, 0x7, 0x0, 0x70, 0x6,
    ...
    0x18, 0x7, 0x80, 0x78, 0xe, 0x0, 0xe0, 0x0, 0x0, 0x0, 0x0, 0x7, 0xc0, 0x74, 0x1e, 0x9, 0xe1, 0x98,
...
uint8_t get_short_from_color_pattern(uint8_t n, uint8_t color_pattern_number) {
  // A color pattern block is 73 bytes, headed by a "max" byte
  uint8_t ColorPatternMatrix[] = {
    ...
    
  };
  color_pattern_number = color_pattern_number % NUM_COLOR_PATTERNS;
  uint8_t pattern_short = ColorPatternMatrix[(color_pattern_number * NUM_LEDS / 2) + 1 + (n / 2)];
  if (n % 2) {
    pattern_short = pattern_short >> 4;
  }
  return (pattern_short & 0x0f);
}
