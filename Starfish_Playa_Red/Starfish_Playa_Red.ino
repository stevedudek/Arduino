...
//  Starfish
///
//  No xBee. Red colors. Slower transitions.
//
...
//
//  Turned off HSV correction - it may cause blinking (or not)
//  Return to HSV by restoring a line in morph frame routine
//
/*****************************************************************************/
//
// Set Up Pins
...
void morph_frame() {
   for (int i = 0; i < numLights; i++) {
     // Turning off HSV correction  in line below - it may be the cause of white flashing
...
int GetDelayTime(int wait) {
  ...
  return (5 * DelayValues[wait % MAX_WAIT]);
}
