...
//
//  Two bunny ears - 4 part symmetric
//
//  Modern software
//
//  3/23/24
//
#define NUM_LEDS    12
#define ACTUAL_LEDS 52 // 6 + 7 + 7 + 6 + 6 + 6 + 6 + 1
...
void setup() {
  ...
  FastLED.addLeds<WS2812B, DATA_PIN, GRB>(leds, ACTUAL_LEDS);  // Only 1 leds object
 ...
 boolean update_leds = false;
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    if (led_buffer[i].h == color.h && led_buffer[i].s == color.s && led_buffer[i].v == color.v) {
      ...
      // 4 part symmetry for Bunny Ears
      leds[i + 1] = rgb_color;
      leds[i + 27] = rgb_color;
      leds[24 - i] = rgb_color;
      leds[50 - i] = rgb_color;
