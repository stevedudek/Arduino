...
#define PIXEL_X_SPACING  (255 / (MAX_XCOORD - 1))
#define PIXEL_Y_SPACING  (255 / (MAX_YCOORD - 1))
...
void patterns(uint8_t c) {
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 0; y < MAX_YCOORD; y++) {
      boolean value = get_bit_from_pattern_number((y * MAX_XCOORD) + x, show_variables[c]);
...
uint8_t rotate_pixel(uint8_t i, uint8_t channel) {
  uint8_t x = i % MAX_XCOORD;
  uint8_t y = i / MAX_YCOORD;
  ...
  return ((y * MAX_XCOORD) + x) % NUM_LEDS;
}
