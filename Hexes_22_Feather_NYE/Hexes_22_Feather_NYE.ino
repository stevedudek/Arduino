...
void game_of_life(uint8_t c, boolean two_color) {
  ...
  shows[c].setWait(map8(show_speed, 20, 60));
...
void vert_back_forth_dots(uint8_t c) {
  uint8_t temp_x;
  uint16_t cycle = shows[c].getCycle() / map8(show_speed, 4, 1);
...
void diag_back_forth_dots(uint8_t c) {
  ...
  uint16_t cycle = shows[c].getCycle() / map8(show_speed, 6, 2);
...
String getReadings (uint8_t c) {
  ...
  jsonReadings["sv"] = show_var[c];  // New for Hexes
  jsonReadings["p"] = pattern[c];
  jsonReadings["m"] = pattern[c + 2];
  jsonReadings["fill"] = pattern[c + 4];
  jsonReadings["wp"] = pattern[c + 6];
...
void processMeshMessage(JSONVar myObject) {
  ...
  show_var[c] = int(myObject["sv"]);  // New for Hexes
  pattern[c] = int(myObject["p"]);
  pattern[c + 2] = int(myObject["m"]);
  pattern[c + 4] = int(myObject["fill"]);
  pattern[c + 6] = int(myObject["wp"]);
