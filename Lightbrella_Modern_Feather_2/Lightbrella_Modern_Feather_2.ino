...
//  Modernized
//
#define NUM_LEDS 29  // Chance of memory shortage for large NUM_LEDS
#define ACTUAL_LEDS (30 * 8)  // Chance of memory shortage for large NUM_LEDS
...
void setup() {
  ...
  FastLED.addLeds<WS2812B, DATA_PIN>(leds, ACTUAL_LEDS);  // Only 1 leds object
...
void morph_channels(uint8_t fract) {
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    light_spokes(i, color);
    led_buffer[i] = color;
  }
  light_spokes(29, CHSV(0,0,0));
}

void light_spokes(uint8_t i, CHSV color) {
  for (uint8_t s = 0; s < 8; s++) {
    if (s % 2) {
      leds[(s * 30) + (NUM_LEDS - i - 1)] = color;
    } else {
      leds[(s * 30) + i] = color;
    }
  }
