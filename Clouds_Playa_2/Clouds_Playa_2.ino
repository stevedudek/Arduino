 ...
 *   8/19/17
 *
 *   56 Clouds controlled by a black box
 *
 *   3 knobs, 3 buttons, and an Arduino (no xBee)
 *
 ******************/

#define numClouds 56

float brightness = 1;  // A fraction: 0.0 - 1.0
#define MAX_BRIGHTNESS 0.25  // BRIGHTNESS MAY NEED TO BE CAPPED AT 0.25 for a 3A power supply;
...
// Shows
int curr_show = 0;  // Starting show (0 = AllOn)
...
int LEDChannels[numClouds][3] = {0};    // 8 for each octobar

// Delays
#define DELAY 20 // Only delay in the system (in ms)
#define MACETECH_DELAY1 2
#define MACETECH_DELAY2 2
...
byte cloud_map[numClouds][2] = {
  { 0, 0 }, { 1, 1 }, { 2, 2 }, { 3, 3 }, { 4, 4 }, // 0 - 4
  { 5, 5 }, { 6, 7 }, { 7, 7 }, { 8, 8 }, { 9, 9 }, // 5 - 9
 
  { 10, 10 }, { 11, 11 }, { 12, 12 }, { 13, 13 }, { 14, 14 }, // 10 - 14
  { 15, 15 }, { 16, 16 }, { 17, 17 }, { 18, 18 }, { 19, 19 }, // 15 - 19
  
  { 20, 20 }, { 21, 21 }, { 22, 22 }, { 23, 23 }, { 24, 24 }, // 20 - 24
  { 25, 25 }, { 26, 26 }, { 27, 27 }, { 28, 28 }, { 29, 29 }, // 25 - 29
  
  { 30, 30 }, { 31, 31 }, { 32, 32 }, { 33, 33 }, { 34, 34 }, // 30 - 34
  { 35, 35 }, { 36, 36 }, { 37, 37 }, { 38, 38 }, { 39, 39 }, // 35 - 39

  { 40, 40 }, { 41, 41 }, { 42, 42 }, { 43, 43 }, { 44, 44 }, // 40 - 44
  { 45, 45 }, { 46, 46 }, { 47, 47 }, { 48, 48 }, { 49, 49 }, // 45 - 49

  { 50, 50 }, { 51, 51 }, { 52, 52 }, { 53, 53 }, { 54, 54 }, // 50 - 54
  { 55, 55 }
...
boolean locked = true;  // Whether the box can be adjusted or not

byte buttonpins[NUM_BUTTONS] = { 11, 9, 10 };  // Digital pins for buttons 1, 2, 3
...
#define MIN_CHANGE 10 // Minimum change in a dial that will register. Prevents flickering
...
void loop() {
  ...
  if (!random(10)) {
    foreColor = IncColor(foreColor, 100);
  }
...
uint32_t Gradient_Wheel(int color, float intensity)
{
  ...
  color = restrict_color(color);
  
  channel = color / 1024;
  value = color % 1024;

  intensity *= (brightness * MAX_BRIGHTNESS);
...
void check_box() {
  ...
  if (abs(dial - dialval[COLOR_DIAL]) > MIN_CHANGE) { // Has dial #1 budged?
    ...
    Serial.println(constrain_color);
  }
  ...
  // PREVIOUS_BUTTON
  if (ButtonPushed(PREVIOUS_BUTTON)) {
    curr_show = (curr_show - 1 + MAX_SHOW) % MAX_SHOW;
    morph = 0;
    cycle = 0;
    shuffle_clouds();
    update = true;
...
void write_clouds() {
    ...
    delayMicroseconds(MACETECH_DELAY1);
    digitalWrite(latchpin,HIGH); // latch data into registers
    delayMicroseconds(MACETECH_DELAY2);
    digitalWrite(latchpin,LOW);
