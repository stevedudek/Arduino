...
void next_show() {
  ...
  if (current_show == 0) {
    ...
    current_show = random8(2, NUM_SHOWS);
  } else {
    current_show = 0;
  }
...
void sawtooth() {
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    setFishPixelColor(NUM_LEDS-i-1, Gradient_Wheel(foreColor, fract));
  }  
}
...
void color_gradient_show() {
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    // "i" will have pattern move up; "NUM_LEDS-i-1'' will have pattern move down
    setFishPixelColor(i, Wheel(interpolate_wrap(foreColor, backColor, fract)));
  }
}
...
void bands() {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    if (wave > 0) {
      setFishPixelColor(NUM_LEDS-i-1, CHSV(foreColor, 255, wave) );
    }
...
void noisy_show() {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    setFishPixelColor(i, CHSV(noise[0][i], 255, noise[1][NUM_LEDS-i-1]));
  }
}
...
void plinko() {
  ...
  for (i = 0; i < NUM_PLINKO; i++) {
    if (plink[i] != -1) {
      setFishPixelColor(plink[i], Wheel(IncColor(foreColor, 5 * i))); // Draw the plinko
    }
  }
}
...
//
//  set Fish Pixel Color - make sure tail + head look right
//
//  Head is (1). LEDs 0-2-3 will display along with (1).
//  For the tail: 55 -> 54, 57 -> 56, 59 -> 58
//
void setFishPixelColor(uint8_t i, CHSV color) {
  if (i == 0 || i == 2 || i == 3 || i == 55 || i == 57 || i == 59) {
    return;  // ignore duplicate head + tail pixels
  } else if (i == 1) {
    for (uint8_t head = 0; head < 4; head++) { 
      next_frame[head] = color; 
    }
  } else if (i == 54 || i == 56 || i == 58) {
    next_frame[i] = color;
    next_frame[i+1] = color; 
  } else {
    next_frame[i] = color;
  }
}
...
void addPixelColor(int i, CHSV c2) {
  ...
  if (c1.v > c2.v) {
    setFishPixelColor(i, c1);
  } else {
    setFishPixelColor(i, c2);
  }
}
...
uint8_t get_two_bits_from_pattern_number(uint8_t n, uint8_t pattern_number) {
  uint8_t pattern_byte = pattern_matrix[pattern_number][(n / 4)];
  return (pattern_byte & (B11000000 >> (2 * (n % 4)) ));
}
