...
long last_time;
#define SMOOTHING 5   // 0 = no smooth, lower the number = more smoothing
...
CHSV* leds_old_buffer = new CHSV[NUM_LEDS];
...
void morph_channels(uint8_t fract) {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    if (led_number != XX) {
      ...
      if (SMOOTHING > 0) {
        color = led[CHANNEL_A].smooth_color(leds[led_number], color, SMOOTHING);  // smoothing
      }
      leds[led_number] = color;
      leds_old_buffer[led_number] = color;
