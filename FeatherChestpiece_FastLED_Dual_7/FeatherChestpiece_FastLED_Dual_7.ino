...
#define DATA_PIN  0  // 7  // 0
#define CLOCK_PIN  2  // 8  // 2
...
uint8_t PetalLookUp[12] = { 0,2,3,5,6,7,8,9,10,12,13,15 };
#define NUM_DARK  4
...
void setup() {
  ...
  Serial.begin(115200);
  Serial1.begin(115200);  // Serial1: xBee port
...
void morph_channels(uint8_t fract) {
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    led_buffer[i % NUM_LEDS] = color;
    leds[lookup_petal(i % NUM_LEDS)  % TOTAL_LEDS] = narrow_palette(color);  // Send color to the actual LED
  }
}
