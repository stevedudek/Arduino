...
int GetDelay(float delaytime) {  // If pot turned off, returns a default delay - otherwise returns a scaled analog value
  if (analogRead(PotPin) >= HighVolt) return delaytime; else return ((5*delaytime*analogRead(PotPin))/1023);
}
