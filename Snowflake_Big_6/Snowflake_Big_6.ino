...
//  10/3/2018
//
//  TEST SWITCH_AXIS! With lightrunup show (#7). Check inversion and different symmetries.
//
//  xBee - Speaking & Hearing Modes
//

#define SNOWFLAKE_SIZE 2  // 0 = Small, 1 = Medium, 2 = Large
...
void pick_symmetry() {
  ...
  switch_axis = random8(2) ? true : false;
  set_symmetry();
}
...
uint8_t try_axis_switch(uint8_t i) {
  ...
  if (i < 6) {
    ...
    i -= 6;
    offset = (i / SPOKE_LENGTH) + 1;  // 12 might mean SPOKE_LENGTH and vice versa
    i = i % SPOKE_LENGTH;
