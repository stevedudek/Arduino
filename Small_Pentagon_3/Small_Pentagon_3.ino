...
uint8_t current_pattern[] = { 0, 0, 0, 0 };
...
uint8_t get_pixel_from_coord(uint8_t x, uint8_t y) {
  return ((y % MAX_YCOORD) * MAX_XCOORD) + (x % MAX_XCOORD);
}
...
uint8_t get_pos_distance(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2) {
  ...
  uint8_t dx = map8(abs(x2 - x1), 0, 180);
  uint8_t dy = map8(abs(y2 - y1), 0, 180);
...
void morph_channels(uint8_t fract) {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    if (led_number != XX) {
      CHSV color = led[CHANNEL_A].getInterpHSV(leds_buffer[CHANNEL_B][rotate_pixel(i, CHANNEL_B)], 
...
uint8_t rotate_pixel(uint8_t i, uint8_t channel) {
  ...
  uint8_t rotation = current_pattern[2 + channel];  // Very strange memory bug here
