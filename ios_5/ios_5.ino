...
//  mesh.onReceive(&receivedCallback);
  ...
  WiFi.softAP(APSSID);  // You may need to omit the password
...
void handleControl() {
  if (server.hasArg(BRIGHTNESS_NAME)) {
    ...
    server.send(200, "text/html", "<h1>Brightness</h1>");
  }
  
  if (server.hasArg(HUE_NAME)) {
    sendParameterMessage(HUE_COMMAND, server.arg(HUE_NAME).toInt());
    server.send(200, "text/html", "<h1>Hue</h1>");
  }
  
  if (server.hasArg(HUE_WIDTH_NAME)) {
    sendParameterMessage(HUE_WIDTH_COMMAND, server.arg(HUE_WIDTH_NAME).toInt());
    server.send(200, "text/html", "<h1>Hue width</h1>");
  }
  
  if (server.hasArg(SPEED_NAME)) {
    sendParameterMessage(SPEED_COMMAND, server.arg(SPEED_NAME).toInt());
    server.send(200, "text/html", "<h1>Speed</h1>");
  }
  
  if (server.hasArg(SHOW_DURATION_NAME)) {
    sendParameterMessage(SHOW_DURATION_COMMAND, server.arg(SHOW_DURATION_NAME).toInt());
    server.send(200, "text/html", "<h1>Duration</h1>");
  }
  
  if (server.hasArg(FADE_NAME)) {
    sendParameterMessage(FADE_COMMAND, server.arg(FADE_NAME).toInt());
    server.send(200, "text/html", "<h1>Fade</h1>");
  }
}
