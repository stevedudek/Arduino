...
//
//  New algorithm: double the leds object (may be too big a hit on memory, but simplifies code)
...
CRGB leds[DUAL][NUM_LEDS];  // 2 leds objects! Hook for FastLED library.
...
void setup() {
  ...
  FastLED.addLeds<WS2801, DATA_PIN, CLOCK_PIN>(leds[CHANNEL_A], NUM_LEDS);
...
//
// interpolate_leds - interpolate the two leds[2] object and push result on to leds[Channel_A]
//
void interpolate_leds() {
  ...
  if (intensity_a == 0) {
    for (int i = 0; i < NUM_LEDS; i++) {  // Channel A is off. Push Channel B on to Channel A.
      leds[CHANNEL_A][i] = leds[CHANNEL_B][i];
    }
  } else if (intensity_b == 0) {
    return;  // Channel B is off. Use only Channel A (already loaded).
  } else {
...
void morph_channels(uint8_t fract) {
  for (int i = 0; i < NUM_LEDS; i++) {
    // RGB interpolation
    leds[CHANNEL_A][i] = led[CHANNEL_A].getInterpRGB(leds[CHANNEL_A][i], leds[CHANNEL_B][i], fract);  // interpolate a + b channels

    // TRY NEXT: 2 HSV records for A + B
    /*
    // Sadly, rgb->hsv interpolation is noticeably jerky: perhaps check for black before proceding
    // RGB -> HSV -> Interpolate -> HSV -> push on to Channel A
    leds[CHANNEL_A][i] = led[CHANNEL_A].getInterpHSV(led[CHANNEL_A].rgb_to_hsv(leds[CHANNEL_A][i]), 
                                                     led[CHANNEL_B].rgb_to_hsv(leds[CHANNEL_B][i]), 
...
//
//  Simple RGB interpolation between two RGB colors
//
CRGB getInterpRGB(CRGB c1, CRGB c2, uint8_t fract)
{
  return CRGB(lerp8by8(c1.r, c2.r, fract),
              lerp8by8(c1.g, c2.g, fract),
              lerp8by8(c1.b, c2.b, fract)
             );
}
...
void draw_center(uint8_t channel) {
  leds[channel][NUM_LEDS-1] = led[channel].getInterpFrameColor(0);
}
...
void draw_hub(uint8_t channel) {
  ...
  for (uint8_t i = 0; i < pitch; i++) {
    for (uint8_t j = 0; j < symmetries[symmetry[channel]]; j++) {
      ...
      leds[channel][pixel] = led[channel].getInterpFrameColor(1 + i);
    }
  }
}
  ...
  if (symm < 4) {
    ...
    for (uint8_t i = 0; i < pitch; i++) {      
      for (uint8_t j = 0; j < symmetries[symm]; j++) {
        ...
        leds[channel][pixel] = led[channel].getInterpFrameColor(hub_pixels[symm] + i);
      }
    }
  } else {  // For 12-fold symmetry
    for (uint8_t i = 0; i < ((SPOKE_LENGTH/2)+1); i++) {
      for (uint8_t spoke = 0; spoke < 6; spoke++) {
        leds[channel][shift_one((SPOKE_LENGTH - i) + (spoke * SPOKE_LENGTH))] = led[channel].getInterpFrameColor(hub_pixels[symm] + i);
        leds[channel][shift_one(i + (spoke * SPOKE_LENGTH))] = led[channel].getInterpFrameColor(hub_pixels[symm] + i);        
      }
    }
  }
}
