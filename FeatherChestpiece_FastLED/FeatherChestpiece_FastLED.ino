...
void setup() {
  ...
  FastLED.addLeds<WS2801, DATA_PIN, CLOCK_PIN>(leds, NUM_LEDS + 4);
...
void check_fades() {
  ...
  if (fade_amount > 0) {
    for (int i = 0; i < NUM_LEDS; i++) {
      leds[PetalLookUp[i]].fadeToBlackBy(fade_amount);
    }
  }
}
...
{ 
  if (c1 == c2) {
    leds[PetalLookUp[i]] = constrain_palette(i, c1);
  } else if (fract == 0) {
    leds[PetalLookUp[i]] = constrain_palette(i, c1);
    return;
  } else if (fract == 255) { 
    leds[PetalLookUp[i]] = constrain_palette(i, c2);
    return;
  } else if (is_black(c1)) {
    leds[PetalLookUp[i]] = constrain_palette(i, c2);
    leds[PetalLookUp[i]].fadeToBlackBy(255 - fract);
    return;
  } else if (is_black(c2)) {
    leds[PetalLookUp[i]] = constrain_palette(i, c1);
    leds[PetalLookUp[i]].fadeToBlackBy(fract);
    return;
  } else {
    leds[PetalLookUp[i]] = constrain_palette(i, CHSV(interpolate_wrap(c1.h, c2.h, fract), 
