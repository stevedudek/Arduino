...
//  FastLED
//
//  Libracized everything!!!
//
//  To Do:
//
//  1. Check this works
//  2. Propagate to Fish Speak
//  3. Propagate to Fish Hear
//  4. Grid: Fish Speak
//  5. Grid: Fish Hear
//
#define FISH_NUM   5
...
void vert_back_forth_bands() {
  ...
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 1; y < MAX_YCOORD; y++) {  // Don't light the head (row 0)
      ...
      intensity = sin8_C(map(temp_x, 0, MAX_XCOORD, 0, 255) + (cycle % (255 / MAX_XCOORD)) );
...
void vert_back_forth_colors() {
  ...
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 0; y < MAX_YCOORD; y++) {
      temp_x = (y % 2) ? x : MAX_XCOORD - x - 1;
      temp_x = (temp_x + cycle) % MAX_XCOORD;
      hue = sin8_C(map(temp_x, 0, MAX_XCOORD, shows.getForeColor(), shows.getBackColor()) + (cycle % (255 / MAX_XCOORD)) );
...
void horiz_back_forth_bands() {
  ...
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 1; y < MAX_YCOORD; y++) {  // Don't light the head (row 0)
      ...
      intensity = sin8_C(map(temp_y, 0, MAX_YCOORD, 0, 255) + (cycle % (255 / MAX_YCOORD)) );
      led.setPixelColor(led.getLedFromCoord(x,y), led.gradient_wheel(shows.getBackColor(), intensity));
    }
  }
  shows.setPixeltoForeColor(1);  // Light up the head
...
void horiz_back_forth_colors() {
  ...
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 0; y < MAX_YCOORD; y++) {
      temp_y = (x % 2) ? y : MAX_YCOORD - y - 1;
      temp_y = (temp_y + cycle) % MAX_YCOORD;
      hue = sin8_C(map(temp_y, 0, MAX_YCOORD, shows.getForeColor(), shows.getBackColor()) + (cycle % (255 / MAX_YCOORD)) );
      led.setPixelHue(led.getLedFromCoord(x,y), hue);
    }
  }
}
...
void diag_back_forth_bands() {
  ...
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 1; y < MAX_YCOORD; y++) {  // Don't light the head (row 0)
      ...
      intensity = sin8_C(map(temp, 0, MAX_YCOORD, 0, 255) + (cycle % (255 / MAX_YCOORD)) );
      led.setPixelColor(led.getLedFromCoord(x,y), led.gradient_wheel(shows.getBackColor(), intensity));
    }
  }
  shows.setPixeltoBackColor(1);  // Light up the head
...
void diag_back_forth_colors() {
  ...
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 0; y < MAX_YCOORD; y++) {
      ...
      temp_y = (x % 2) ? y : MAX_YCOORD - y - 1;
      temp = (temp_x + temp_y + cycle) % MAX_YCOORD;
      hue = sin8_C(map(temp, 0, MAX_YCOORD, shows.getForeColor(), shows.getBackColor()) + (cycle % (255 / MAX_YCOORD)) );
      led.setPixelHue(led.getLedFromCoord(x,y), hue);
    }
  }
}
