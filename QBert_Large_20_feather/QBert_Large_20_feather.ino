...
#define MAX_LUMENS 40000  // estimated maximum brightness until flickering. 2A should allow 25500.

#define DELAY_TIME 10  // in milliseconds. 10 is on the edge: watch out for jittering
...
// Shows
#define NUM_SHOWS 20
#define START_SHOW_CHANNEL_A  10  // Startings shows for Channels A + B
...
void windmill(uint8_t i) {
  ...
  for (uint8_t n = 0; n < NUM_LEDS; n++) {
    uint8_t value = beatsin8(beat_freq - (get_ycoord(n) / 10), 0, 255, 0, get_xcoord(n));
...
void center_ring(uint8_t c) {
  ring(c, shows[c].getForeColor(), shows[c].getForeBlack(), 128, 128, map8(show_speed, 1, 4), 64, 64);
}
...
void processPhoneMessage(JSONVar myObject) {
  processPhoneParam(int(myObject["param1"]), int(myObject["value1"]));
  processPhoneParam(int(myObject["param2"]), int(myObject["value2"]));
}

void processPhoneParam(uint8_t param, uint8_t value) {
