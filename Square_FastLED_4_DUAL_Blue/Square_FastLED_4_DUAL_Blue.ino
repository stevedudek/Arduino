...
// Shows
#define NUM_SHOWS 16
#define START_SHOW_CHANNEL_A  14  // Startings shows for Channels A + B
...
// Triangle
uint8_t CORNER_AMOUNTS[(MAX_XCOORD + 1) * (MAX_YCOORD + 1)];
uint8_t tri_point_x[3];
uint8_t tri_point_y[3];
int8_t tri_speed_x[3];
int8_t tri_speed_y[3];
...
// Triangle
//
void triangle(uint8_t i) {
  if (shows[i].isShowStart()) {
//    pick_starting_triangle();
    pick_defined_starting_triangle();
    pick_triangle_speeds();
  }
  move_points();
  calculate_corners();
  draw_triangle(i);
...
void calculate_corners() {
  for (uint8_t x = 0; x < MAX_XCOORD + 1; x++) {
    for (uint8_t y = 0; y < MAX_YCOORD + 1; y++) {
      uint8_t corner_amount = 0;
      uint8_t corner_x = map(x, 0, MAX_XCOORD, 0, 255);
      uint8_t corner_y = map(y, 0, MAX_XCOORD, 0, 255);
      corner_amount += same_side(corner_x, corner_y, tri_point_x[0], tri_point_y[0], tri_point_x[1], tri_point_y[1], tri_point_x[2], tri_point_y[2]);
      corner_amount += same_side(corner_x, corner_y, tri_point_x[1], tri_point_y[1], tri_point_x[0], tri_point_y[0], tri_point_x[2], tri_point_y[2]);
      corner_amount += same_side(corner_x, corner_y, tri_point_x[2], tri_point_y[2], tri_point_x[0], tri_point_y[0], tri_point_x[1], tri_point_y[1]);
      set_corner_amount(x, y, corner_amount);
    }
  }
}

uint8_t same_side(uint8_t x_quest, uint8_t y_quest, uint8_t x_in, uint8_t y_in, uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2) {
  // Draw a line through (x1, y1) and (x2, y2)
  // Is (x_quest, y_quest) on the same side of that line as (x_in, y_in)?
  float m = (x2 != x1) ? float(y2 - y1) / (x2 - x1) : MAX_XCOORD ;
  float b = y1 - (m * x1);

  boolean side = (y_in >= ((m * x_in) + b));
  boolean quest = (y_quest >= ((m * x_quest) + b));
  
  return (side ^ quest) ? 0 : 1 ;  // not XOR
}

void draw_triangle(uint8_t i) {
  for (uint8_t y = 0; y < MAX_YCOORD; y++) {
    for (uint8_t x = 0; x < MAX_XCOORD; x++) {
      uint8_t corner_count = add_corners(x, y);
      uint8_t value = (corner_count >= 9) ? map(corner_count, 9, 12, 0, 255) : 0;
      led[i].setPixelColor(get_pixel_from_coord(x,y), CHSV(shows[i].getForeColor(), 255, value) );
...
void move_points() {
  // Adjust the 3 triangle points by their tri_speeds, bouncing off walls
  int16_t new_point;
  for (uint8_t i = 0; i < 3; i++) {
    new_point = tri_point_x[i] + tri_speed_x[i];
    if (new_point < 0 || new_point > 255) {  // off the board
      tri_speed_x[i] *= -1;
    } else {
      tri_point_x[i] = new_point;
    }
    new_point = tri_point_y[i] + tri_speed_y[i];
    if (new_point < 0 || new_point > 255) {  // off the board
      tri_speed_y[i] *= -1;
    } else {
      tri_point_y[i] = new_point;
    }
  }
}

uint8_t add_corners(uint8_t x, uint8_t y) {
  uint8_t corner_count = 0;
  corner_count += get_corner_amount(x,   y);
  corner_count += get_corner_amount(x,   y+1);
  corner_count += get_corner_amount(x+1, y);
  corner_count += get_corner_amount(x+1, y+1);
  return corner_count;
}

uint8_t get_corner_amount(uint8_t x, uint8_t y) {
  return CORNER_AMOUNTS[(y * (MAX_YCOORD+1)) + x];
}

void set_corner_amount(uint8_t x, uint8_t y, uint8_t value) {
  CORNER_AMOUNTS[(y * (MAX_YCOORD+1)) + x] = value;
}

void print_corner_amounts() {
  for (int8_t y = MAX_YCOORD; y >= 0; y--) {
    for (uint8_t x = 0; x < MAX_XCOORD + 1; x++) {
      Serial.print(get_corner_amount(x, y));
      Serial.print(",");
    }
    Serial.println("");
  }
...
void pick_defined_starting_triangle() {
  tri_point_x[0] = 0;
  tri_point_y[0] = 0;
  tri_point_x[1] = 128;
  tri_point_y[1] = 255;
...
void pick_starting_triangle() {
  uint8_t not_corner = random(4);  // 3 triangle corners, one quadrant empty
  uint8_t i = 0;
  for (uint8_t quad = 0; quad < 4; quad++) {
    if (quad != not_corner) {
      uint8_t x = random(128);
      uint8_t y = random(128);
      if (quad % 2) {  // 1 and 3
        x += 128;
      }
      if (quad > 1) {  // 2 and 3
...
void pick_triangle_speeds() {
  for (uint8_t i = 0; i < 3; i++) {
    tri_speed_x[i] = get_random_speed();
    tri_speed_y[i] = get_random_speed();
  }
}

int8_t get_random_speed() {
  int8_t spd = random(0,5);
  if (random(2)) spd *= -1;
  return spd;
}
