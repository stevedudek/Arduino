...
// Shows
#define START_SHOW_CHANNEL_A   25  // Channels A starting show
...
uint8_t get_random_symmetry() {
  ...
  return symmetry_lookup[random(5)];
}
...
void cone_push(uint8_t channel) {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    uint8_t value = max(48, beatsin8(10, 64, 255, 0, dist));
...
void windmill(uint8_t i) {
  if (shows[i].isShowStart()) {
    set_spare_show_variable(random(10, 40), 0, i);
    set_spare_show_variable(random(MAX_SPOKE, MAX_DISTANCE), 1, i);
    shows[i].turnOffMorphing();
  }
  
  for (uint8_t x = 0; x < MAX_SPOKE; x++) {
    for (uint8_t y = 0; y < MAX_DISTANCE; y++) {
      uint8_t value = beatsin8(get_spare_show_variable(0, i) - (4 * y), 0, 255, 0, x * 128 / get_spare_show_variable(1, i) );
...
void pendulum_wave(uint8_t i, boolean smoothed) {
  ...
  if (shows[i].isShowStart()) {
    set_spare_show_variable(MAX_SPOKE + 1, 0, i);
    shows[i].turnOffMorphing();
  }
  
  for (uint8_t x = 0; x < MAX_SPOKE; x++) {
    value = beatsin8(get_spare_show_variable(0, i) - x);
    for (uint8_t y = 0; y < MAX_DISTANCE; y++) {
...
void plinko(uint8_t c, boolean in_direction) {

  // Refresh plinkos at the start of the show
  if (shows[c].isShowStart()) {
    shows[c].setWait(1);
    no_mask(c);
    set_spare_show_variable(random(1, 6), 0, c);  // frequency a new plinko starts
  ...
  for (uint8_t i = 0; i < get_spare_show_variable(0, c); i++) {
    ...
    if (new_index != XX) {
      ...
      uint8_t new_hue = shows[c].IncColor(shows[c].getForeColor(), random(0, 60));
  ...
  for (uint8_t i = 0; i < MAX_PLINKO; i++) {
    if (get_plinko_s(i, c) != XX) {
      Serial.print(get_plinko_s(i, c));
      Serial.print(", ");
      Serial.println(get_plinko_d(i, c));
