...
// framebuffers
uint32_t current_frame[NumLEDs];  // 32-bit color value
uint32_t next_frame[NumLEDs];     // 32-bit color value
...
uint32_t LEDChannels[NumLEDs][3] = {0};

unsigned long time = 1;         // Keeps track of time
unsigned long butt1time = 1;    // Button 2 time
unsigned long butt2time = 1;    // Button 2 time

int show = 0; // Which show we start on
#define MaxShow  5  // Total number of shows

int currDelay = 500;  // Delay in milliseconds between animation
#define MinDelay 100  // Minimum delay
#define MaxDelay 5000  // Maximum delay

uint16_t color1 = 500;   // Foreground color
uint16_t color2 = 500;   // Background color
...
void OneLightRunDown() {
  ...
  for (int i=0; i < NumLEDs; i++) {
    if (i == onLight) { LightFlower(i,color1); }
    else { LightFlower(i, color2); }
  }
}
...
void BlackWave() {
  ...
  */
  ...
  for (int i=0; i < NumLEDs; i++) { 
    if (WhichOn[i]) { LightFlower(i, color1); } else { LightFlower(i,color2); }
  }
}
...
void AlternateLights() {
  for (int i=0; i < NumLEDs; i++) {
    if ((i%2)^(time%2)) { LightFlower(i, color1); } // Exclusive Or: tricky!
 ...
 * Light a Flower
 *
 * Put 3071-color into a particular numbered flower
 * Values go into the next_frame array
 * Does not light up the array
 */

void LightFlower(int flower, uint16_t color) {
...
void morph_frame(int msec) {
  ...
  for (int i = 0; i < NumLEDs; i++) {    
    for (int j = 0; j < 3; j++) {
      uint16_t old_color = (current_frame[i] >> (j * 10)) & 0x3ff; 
      uint16_t new_color = (next_frame[i] >> (j * 10)) & 0x3ff;
  ...
  for (t = 1; t < steps; t++) {
    for (int i = 0; i < NumLEDs; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        uint16_t old_color = (current_frame[i] >> (j * 10)) & 0x3ff;
        color |= ((unsigned long)(old_color + (deltas[i][j] * t / steps)) & 0x3ff) << (j * 10);
      }
  ...
  if(!random(chance)) { color1 = (color1 + 20) % MaxColor; }
  if(!random(chance)) { color2 = (color2 - 10) % MaxColor; }
  if(!random(chance*2)) { currDelay = (currDelay + 25) % MaxDelay; } 
