...
// Clocks and time
#define SHOW_DURATION 50  // seconds
...
uint8_t effect_number[] = { 0, 0 };  // 0 = off, 1 = hue, 2 = sat, 3 = value
uint8_t EffectBeatsPerMinute[] = { MIN_EFFECTS_BEATS, MIN_EFFECTS_BEATS };
uint8_t center_pixel[] = { 0, 0 };
...
void setup() {
  ...
  shows[CHANNEL_A].setWait(25); // For debugging
...
//  morph_channels(255);  // morph together the 2 leds channels and deposit on to Channel_A
...
void hsvWave(uint8_t effect, uint8_t i) {
  if (shows[i].isShowStart()) {
//    effect_number[i] = effect;
    effect_number[i] = 3;
    EffectBeatsPerMinute[i] = MIN_EFFECTS_BEATS;
//    EffectBeatsPerMinute[i] = random8(MIN_EFFECTS_BEATS, MAX_EFFECTS_BEATS);
//    center_pixel[i] = random8(NUM_LEDS);
    center_pixel[i] = 11;
  }
}

void test_chain(uint8_t channel) {
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    if (i == shows[channel].getCycle() % NUM_LEDS) {
...
void morph_channels(uint8_t fract) {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    if (effect_number[CHANNEL_A] > 0) {  // Effect on Channel A?
      color = add_effect(leds_buffer[CHANNEL_B][i], 
                         get_effect_amount(i, CHANNEL_A), 
                         CHANNEL_A, fract);
    } else if (effect_number[CHANNEL_B] > 0) {  // Effect on Channel B?
      color = add_effect(leds_buffer[CHANNEL_A][i], 
                         get_effect_amount(i, CHANNEL_B), 
                         CHANNEL_B, 255 - fract);
    } else {
...
//
CHSV add_effect(CHSV color, uint8_t effect_amount, uint8_t channel, uint8_t effect_fract) {

  // Convert full 0-255 to something more mild, inverse with speed
  effect_fract = map8(effect_fract, 0, 255 * MIN_EFFECTS_BEATS / EffectBeatsPerMinute[channel]);
  
  switch(effect_number[channel]) {  
...
//      color.s = lerp8by8(color.s, map8(effect_amount, 255, 128), effect_fract);
      color.s = lerp8by8(color.s, effect_amount, effect_fract);
      break;
    case 3:  // value
...
//
uint8_t get_effect_amount(uint8_t i, uint8_t channel) {
  return sin8_C( beatsin8( EffectBeatsPerMinute[channel], 0, 255) + get_distance(i, center_pixel[channel]) );
}
