...
void loop() {
  ...
  switch (show) {
    ...
    case 0: {   // Light running show
      ...
      if (!random(500)) {  // CENTER = add another running light
        if (x++>wingLights) x=1;
      }
      if (!random(250)) { // UP = slow speed
    ...
    case 2: {  // Rainbow show
      ...
      if (!random(500)) { // LEFT = slow speed
        if (wait < maxWait) wait++ ;
      }
      if (!random(500)) { // RIGHT = increase speed
        if (wait > 1) wait--;
      }
      if(!random(500)) { x++; }  // UP = increase color divisions
    ...
    case 3: {   // Lightwaves show
      ...
      if (!random(4)) {       // UP = change color
        if (currColor++ >= MaxColor) currColor = 0;
      }
      if (!random(500)) { // LEFT = slow speed
        if (wait++ >= maxWait) wait=0;
      }
      if (!random(2)) { // RIGHT = change background color
        if (currColor2++ >= MaxColor) currColor2 = 0;
      }
   ...
   case 5: {  // Alternate colors
      ...
      if (!random(4)) {       // LEFT = change foreground color
        if (currColor++ >= MaxColor) currColor = 0;
      }
      if (!random(6)) { // RIGHT = change background color
        if (currColor2++ >= MaxColor) currColor2 = 0;
      }
  ...
  case 6: {  // Color Gradient
      ...
      if (f<0 || f>=wingLights) f=0;  // Sets bounds for frame of show
      if (!random(50)) {     // CENTER = slow speed
        if (wait++ >= maxWait) wait=0;
      }
      if(!random(50)) {          // UP = change start of gradient
        if (currColor++ >= MaxColor) currColor = 0;
      }
      if (!random(25)) {     // RIGHT = increase width of gradient
        if (currColor2++ >= MaxColor) currColor2 = 0;
      }
