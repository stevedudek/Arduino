...
uint8_t section_list[] = { SECTION_B, SECTION_A };
...
#define NUM_SCALES   (97 + 86)  // set this manually
#define NUM_SPACERS  (18 + 21)  // set this manually
...
void test_neighs(uint8_t c) {
  ...
  for (int8_t x = min_x; x < max_x; x++) {
    for (int8_t y = min_y; y < max_y; y++) {
      ...
      /*
      if (scale != XX) {
        shows[i].setPixeltoForeColor(get_scale_for_coord(coord));  // Draw the center
      }
      /*
      for (uint8_t dir = 0 ; dir < 6; dir++ ) {
        shows[i].setPixeltoBackColor(get_scale_for_coord(get_coord_in_dir(dir, coord)));
      }
      */
    }
  }
...
//
uint16_t get_coord_in_dir(uint16_t dir, uint16_t coord) {
