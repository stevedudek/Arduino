...
// CHSV led_buffer[NUM_LEDS];  // For smoothing  NO, can you smooth off of the leds below?
// Smoothing off leds: need to write smooth function for RGB / RGB smoothing
...
void color_by_cart_coord(uint8_t c) {
  ...
  for (uint8_t x = 0; x < MAX_COLUMNS; x++) {
    ...
    for (uint8_t y = y_start; y < y_end; y++) {
      ...
      shows[c].setPixeltoHue(scale, shows[c].IncColor(back_hue, (x * 5) + (y * 2)));
