...
void two_d_color() {
  ...
  for (int x=0; x < light_width; x++) {
    for (int y=0; y < light_height; y++) {
      ...
      strip[pixel].fadeToBlackBy( 256 * MAX_DIM ); // Now fade it
    }
  }
}
...
void morph_frame() {
   ...
   for (int i = 0; i < numLights; i++) {
     ...
     if (current_frame[i] != next_frame[i]) {
       ...
       strip[i].fadeToBlackBy( 256 * MAX_DIM ); // Now fade it
     }
   }
   FastLED.show();
}
...
//
float getDimAmount() {
   float amount = 1.0 - getFractBlack();
   if (amount > MAX_DIM) {
     return MAX_DIM;
   }
   if (amount < MIN_DIM) {
...
// getFractBlack
//
// Returns the 0.0-1.0 fraction of lights whose current_frame is black

float getFractBlack() {
  int black = 0;
  
  for (int i = 0; i < numLights; i++) {
    if (next_frame[i] == BLACK) {
      black++;
    }
  }
  
  return (float)black/numLights;
}
