...
//
//  Seahorse - 7 head lights, 14 back lights, 18 tail lights
//
/*****************************************************************************/
//
// 10/22/14
...
#define numLights 40      // two horn lights oddly handled
...
void clear() {
  for (int i=0; i<numLights; i++) {
    setLookupPixelColor(i, Color(0,0,0));
...
void unit_test() {
  ...
  for (int i=0; i<numLights; i++) {
    clear();
    setLookupPixelColor(i, Color(255,0,0));
    strip.show();
    delay(500);    
  }
}
...
void draw_frame(int delay_ms) {
  for (int i = 0; i < numLights; i++) {
    setLookupPixelColor(i, next_frame[i] & 0xffffff);
...
void morph_frame(int msec) {
  ...
  for (int i = 0; i < numLights; i++) {
    current_frame[i] = next_frame[i];
    setLookupPixelColor(i, current_frame[i]);
  }
  strip.show();
  delay(STEP_LENGTH);
}
...
// setLookupPixelColor
//
// Wrapper for strip.setPixelColor, but
// this function does first
// the appropriate pixel look-up
// i.e. light 2 is both horns
//
void setLookupPixelColor(int pos, uint32_t color) {
  switch (pos) {
    case 0:
      strip.setPixelColor(0, color);
      break;
    case 1:
      strip.setPixelColor(1, color);  // one horn
      strip.setPixelColor(2, color);  // the other horn
      break;
    default:
      strip.setPixelColor(pos+1, color); // +1 accounts for horn
      break;
  }  
}
