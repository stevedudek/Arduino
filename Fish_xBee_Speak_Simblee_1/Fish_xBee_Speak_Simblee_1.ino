...
//
//  Fixes: 
//  changed sin8_avr to sin8
//  Try doing just a patterns show (#0)
//  If still busted: remove the PROGMEM's...
//
//  xBee - Speaking Mode
//
...
#define REPEAT_NUM          2  // How much command duplication
...
//  SimbleeForMobile.advertisementData = "Roman Hat";  // Small name
  ...
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 0; y < MAX_YCOORD; y++) {
      ...
      if (i != -1) {
        temp_x = (y % 2) ? x : MAX_XCOORD - x - 1;
        temp_x = (temp_x + cycle) % MAX_XCOORD;
        hue = sin8(map(temp_x, 0, MAX_XCOORD, foreColor, backColor) + (cycle % (255 / MAX_XCOORD)) );
  ...
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 0; y < MAX_YCOORD; y++) {
      ...
      if (i != -1) {
        temp_y = (x % 2) ? y : MAX_YCOORD - y - 1;
        temp_y = (temp_y + cycle) % MAX_YCOORD;
        hue = sin8(map(temp_y, 0, MAX_YCOORD, foreColor, backColor) + (cycle % (255 / MAX_YCOORD)) );
        setPixelColor(i, Wheel(hue));
      }
    }
  }
  ...
  for (uint8_t x = 0; x < MAX_XCOORD; x++) {
    for (uint8_t y = 0; y < MAX_YCOORD; y++) {
      ...
      if (i != -1) {
        ...
        temp_y = (x % 2) ? y : MAX_YCOORD - y - 1;
        temp = (temp_x + temp_y + cycle) % MAX_YCOORD;
        hue = sin8(map(temp, 0, MAX_YCOORD, foreColor, backColor) + (cycle % (255 / MAX_YCOORD)) );
        setPixelColor(i, Wheel(hue));
      }
    }
  }
...
void repeat_commands(int repeat) {
  // Duplicate commands with a delay in the middle
...
CHSV constrain_palette(CHSV color) {
  ...
  uint8_t hue;
  if (do_palettes) {
    hue = map8(sin8(color.h), palette_center, (palette_center + palette_width) % 256);
...
void set_noise_parameters() {
  ...
  noise_set = random8(ARRAY_SIZE(noise_param_set) / 2);
