...
//
//
#define NUM_LEDS 67  // Chance of memory shortage for large NUM_LEDS
...
uint8_t FADE_TIME = 30;  // seconds to fade in + out (Arduino Blue)
...
void complex_patterns(uint8_t c) {
  ...
  uint8_t pattern = 0; //show_variables[c];
  uint8_t pattern_max = get_complex_max(pattern);
  uint8_t pattern_fill = 0; //show_variables[c + 8];
...
// Geometry
//
void get_coord_from_pixel(uint8_t pixel, uint8_t *s, uint8_t *d) {
  if (pixel == 66) {
    *s = 0;
    *d = 0;
  } else if (pixel >= 30) {
    pixel -= 30;
    uint8_t temp_d = 3 - (pixel / MAX_SPOKE);
    uint8_t temp_s = pixel % 12;
    if (temp_d == 1) {
      temp_s = (temp_s + 1) % MAX_SPOKE;
    }
    *s = temp_s;
    *d = temp_d;
  } else if (pixel >= 6) {
...
void set_leds_by_coord(uint8_t s, uint8_t d, CHSV color) {
  if (d == 0) {
    ...
    uint8_t MIDDLE_LOOKUP[] = { 0, 6, 8, 14, 16, 22, 24, 30, 32, 38, 40, 46 };
    leds[MIDDLE_LOOKUP[s]] = color;
  } else if (d == 4) {
    uint8_t FAR_LOOKUP[] = { 3, 7, 11, 15, 19, 23, 27, 31, 35, 39, 43, 47 };
...
void set_leds_by_pixel(uint8_t pixel, CHSV color) {
  uint8_t s = 0;
  uint8_t d = 0;
  get_coord_from_pixel(pixel, &s, &d);
  set_leds_by_coord(s, d, color);
}
...
void test_ring(uint8_t channel) {
  shows[channel].fillBlack();
  CHSV color = led[channel].wheel(shows[channel].getForeColor());
  uint8_t d = shows[channel].getCycle() % MAX_DISTANCE;
...
void test_spoke(uint8_t channel) {
  shows[channel].fillBlack();
  CHSV color = led[channel].wheel(shows[channel].getForeColor());
  uint8_t s = shows[channel].getCycle() % MAX_SPOKE;
...
uint8_t get_complex_max(uint8_t complex_number) {
  uint8_t complex_max[] = { 12, 17, 17, 9, 13, 6 };
