...
/*  Top Hat
/*
/*  Light sensor turns on and off an LED through a MOSFET
/*
/****/
/*
...
#define LED_PIN 4 // Pin that controls the LED

int ambientValue, proximityValue;
boolean HatState;   // Current state of LED;
...
void loop()
{
  ...
  */
  ...
  if (proximityValue > 2235 ) {   // Does the proximity sensor detect an obstacle?
    if (HatState == true) {       // Yes, is the light on?
      HatState = false;           // Yes, then turn light off (hat is covered)
      digitalWrite(LED_PIN, LOW);
      // Serial.print("Switch Off");
      // Serial.print("\t"); 
    }
  } else {                        // No obstacle detected
    if (HatState == false) {      // Is light off?
      HatState = true;            // Yes, turn light on (hat is uncovered)
