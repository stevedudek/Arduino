...
//  Slave Linear Lights
//
//  Standard program for a linear light strip that listens to commands
//
//  Set the number of lights with numLights below
//  Modern 8-bit color lights
//
//  Responds to an Xbee, but also runs on its own
//
//  Has cycle of show-dark-show-dark
//
...
int show = 1;       // Starting show. Show=0 is dark.
...
void loop() { 
  ...
  // If alone, randomly change foreColor, backColor, show, and timing
  ...
  if (ALONE) {
    ...
    if (!random(10000)) {
      // Toggle
      // If show is dark (0), pick a random show
      // Otherwise go dark
