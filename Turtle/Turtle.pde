...
int show=7;    // default starting show
...
void loop() {
 ...
 key = KeyPad();
  ...
  case 7: {  // Random pixels to test
      ...
      if (key == LEFT) {       // LEFT = decrease number of random pixels
        if (f-- < 0) f = 0;
      }
      if (key == RIGHT) {     // RIGHT = increase number of random pixels
        if (f++ >= numLights) f = numLights;
      }
      
      LightOneLight(random(numLights), currColor, GetDelayTime(wait));
      break;
    }    
  }
}
