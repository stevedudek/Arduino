...
void morph_channels(uint8_t fract) {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    if (led_number != XX) {
      CHSV color_b = led[CHANNEL_B].getInterpFrameColor(rotate_pixel(i, rotate_amount[CHANNEL_B]));
      CHSV color_a = led[CHANNEL_A].getInterpFrameColor(rotate_pixel(i, rotate_amount[CHANNEL_A]));
