...
#define DELAY_TIME 8  // in milliseconds. 10 is on the edge: watch out for jittering
...
//  Serial.begin(9600);
  ...
  FastLED.addLeds<WS2801, DATA_PIN, CLOCK_PIN>(leds, TOTAL_LEDS).setCorrection(TypicalLEDStrip);
...
void updateLeds() {
  ...
  for (uint8_t i = 0; i < DUAL; i++) {
    ...
    shows[i].morphFrame();  // 1. calculate interp_frame
  }
