...
# define MAX_BRIGHTNESS  (BRIGHTNESS * 0.4)
...
void windmill_smoothed(uint8_t i) {
  if (shows[i].isShowStart()) {
    set_spare_show_variable(random8(40, 100), 0, i);
    set_spare_show_variable(random8(MAX_SPOKE, MAX_DISTANCE), 1, i);
    shows[i].turnOffMorphing();
  }
...
void plinko(uint8_t c, boolean in_direction) {
  ...
  for (uint8_t i = 0; i < get_spare_show_variable(0, c); i++) {
    ...
    if (new_index != XX) {
      ...
      set_plinko_position(random8(MAX_SPOKE), start_d, new_index, c);  // s, d, index, c
...
void adjust_brightness() {
  ...
  if (bright_counts > (NUM_LEDS * MAX_BRIGHTNESS)) {
    float dim_ratio = (NUM_LEDS * MAX_BRIGHTNESS) / (float)bright_counts;
    for (int i = 0; i < NUM_LEDS; i++) {
